...
public interface IdentiteDTO extends TableReferenceDTO {
    ...
    public static Comparator<IdentiteDTO> COMPARE = new Comparator<IdentiteDTO>() {
        ...
        public int compare(IdentiteDTO one, IdentiteDTO two) {
            ...
            if (two.getNom().toUpperCase().compareTo(one.getNom().toUpperCase()) == 0) {
                if (
                    two.getPrenom().toUpperCase().compareTo(one.getPrenom().toUpperCase()) == 0 &&
                    two.getDateDebutMandat() != null &&
                    one.getDateDebutMandat() != null
                ) {
                    if (
                        two.getDateDebutMandat().compareTo(one.getDateDebutMandat()) == 0 &&
                        two.getDateFinMandat() != null &&
                        one.getDateFinMandat() != null
                    ) {
                        return two.getDateFinMandat().compareTo(one.getDateFinMandat());
                    } else {
                        return two.getDateDebutMandat().compareTo(one.getDateDebutMandat());
                    }
                } else {
                    return two.getPrenom().toUpperCase().compareTo(one.getPrenom().toUpperCase());
                }
            } else {
                return one.getNom().toUpperCase().compareTo(two.getNom().toUpperCase());
            }
        }
    };
}
