...
public class AfterCaseLinkCreatedListener implements PostCommitEventListener {
    ...
    public void handleEvent(Event event) {
        ...
        final SSRouteStep routeStep = retrieveRouteStep(session, routeStepDocId);
        ...
        if (!VocabularyConstants.ROUTING_TASK_TYPE_INITIALISATION.equals(routeStep.getType())) {
            ...
            profilUtilisateurService.checkAndSendMailNotificationValidationEtape(
                routeStep,
                dossierDoc,
    ...
    private SSRouteStep retrieveRouteStep(final CoreSession session, final String routeStepDocId) {
        final DocumentModel routeStepDoc = session.getDocument(new IdRef(routeStepDocId));
        return routeStepDoc.getAdapter(SSRouteStep.class);
    }
}
