...
public class AfterValidationTransmissionSuppressionListener implements PostCommitEventListener {
    ...
    public void handleEvent(Event event) {
        ...
        try {
            ...
            for (FileUsersSet entryFichierUser : ministereUserMap.values()) {
                ...
                EpgExcelUtil.formatStyleDossier(workbook);
                DataSource fichierExcelResultat = STExcelUtil.convertExcelToDataSource(entryFichierUser.getWorkbook());
                mailService.sendMailWithAttachement(
                    entryFichierUser.getUsersMails(),
                    objet,
                    text,
                    FILE_NAME_TO_SEND,
     ...
     */
    private Map<String, FileUsersSet> initializeFiles(CoreSession session, List<String> dossierIdsList) {
        ...
        for (String dossierId : dossierIdsList) {
            ...
            if (ministereUserMap.containsKey(ministereAttache)) {
                ...
                fileUsersSet = new FileUsersSet(EpgExcelUtil.createDossierExcelFile(), new ArrayList<String>());
                ministereUserMap.put(ministereAttache, fileUsersSet);
            }
            // On ajoute le dossier au workbook correspondant
            EpgExcelUtil.addDossierAtEndToFile(dossierDoc, fileUsersSet.getWorkbook());
        }
     ...
     */
    private void putUsersToListMail(List<STUser> users, Map<String, FileUsersSet> ministereUserMap) {
