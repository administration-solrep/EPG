...
public class ActeServiceImpl implements ActeService {
    ...
    private static final String SUFFIXE_O = "O";

    private static final Map<String, TypeActe> TYPES_ACTES = Collections.unmodifiableMap(initActes());

    /**
     * Liste les types d'actes qui ont des répertoires spécifiques par défaut dans les modèles de fond de dossier.
     */
    private static final Set<String> TYPES_ACTES_EXTEND_FOLDER_FDD = Collections.unmodifiableSet(
        initTypeActeListExtendFolderFondDeDossier()
    );

    /**
     * Liste les types d'actes qui ont des format de fichier défini par défaut dans les modèles de parapheur.
     */
    private static final Set<String> TYPES_ACTES_EXTEND_FORMAT_PARAPH = Collections.unmodifiableSet(
        initTypeActeListExtendFormatParapheur()
    );

    /**
     * Liste les types d'actes dont les dossiers doivent avoir un extrait pour être validés (acte individuel)
     */
    private static final Set<String> TYPES_ACTES_EXTRAIT_OBLIG = Collections.unmodifiableSet(
        initTypeActeListExtraitObligatoire()
    );

    /**
     * Liste les types d'actes pour lesquelles les dossiers seront considérés comme des mesures nominatives
     */
    private static final Set<String> TYPES_ACTES_MESURE_NOMINATIVE = Collections.unmodifiableSet(
        initTypeActeListMesureNominative()
    );

    private static final Set<String> TYPES_ACTES_LOI = Collections.unmodifiableSet(initTypeActeLoiSet());

    private static final Set<String> TYPES_ACTES_ORDONNANCE = Collections.unmodifiableSet(
        Collections.singleton(TypeActeConstants.TYPE_ACTE_ORDONNANCE)
    );

    private static final Set<String> TYPES_ACTES_ARRETE = Collections.unmodifiableSet(initArreteList());

    private static final Set<String> TYPES_ACTES_DECRET = Collections.unmodifiableSet(initDecretList());

    private static final Set<String> TYPES_ACTES_DECRET_PR = Collections.unmodifiableSet(initDecretPRList());

    private static final Set<String> TYPES_ACTES_REGLEMENTAIRE = Collections.unmodifiableSet(initReglementaireList());

    private static final Set<String> TYPES_ACTES_NON_REGLEMENTAIRE = Collections.unmodifiableSet(
        initNonReglementaireList()
    );

    private static final Set<String> TYPES_ACTES_CONVENTION_COLLECTIVE = Collections.unmodifiableSet(
        initConventionCollective()
    );
    /**
     * Sous-ensemble de type pour lesquels il n'y a pas de publication de texte intégral
     */
    private static final Set<String> TYPES_ACTES_NOT_PUBLI_INT = Collections.unmodifiableSet(
        initTypeActeNotVisiblePublicationIntegraleComp()
    );

    /**
     * Sous-ensemble de type pour lesquels il y a visibilté sur le Numéro texte JO
     */
    private static final Set<String> TYPES_ACTES_VISIBLE_NUM_TXT_JO = Collections.unmodifiableSet(
        initTypeActeVisibleNumeroTexteJOComp()
    );

    /**
     * Sous-ensemble de type pour lesquesl il y a visibilité CE
     */
    private static final Set<String> TYPES_ACTES_VISIBLE_CE = Collections.unmodifiableSet(initTypeActeVisibleCEComp());

    /**
     * Sous ensemble de type pour lesquel il y a dossier Saisine rectificative dans fond de dossier
     */
    private static final Set<String> TYPE_ACTES_SAISINE_RECTIFICATIVE = Collections.unmodifiableSet(
        initTypeActeListFDDSaisineRectificative()
    );

    private static final Set<String> TYPES_ACTES_CHEMIN_DE_CROIX = ImmutableSet.of(
    ...
    private static Map<String, TypeActe> initActes() {
        Map<String, TypeActe> actes = new HashMap<>();
    ...
    private static Set<String> initTypeActeListExtendFolderFondDeDossier() {
        Set<String> typesActesExtendFDD = new HashSet<>();
    ...
    private static Set<String> initTypeActeListExtendFormatParapheur() {
        Set<String> typesActesExtendParaph = new HashSet<>();
    ...
    private static Set<String> initTypeActeListMesureNominative() {
        Set<String> typeActeListMesureNominative = new HashSet<>();
    ...
    private static Set<String> initTypeActeLoiSet() {
        Set<String> actesLoi = new HashSet<>();
    ...
    private static Set<String> initArreteList() {
        Set<String> actesArrete = new HashSet<>();
        ...
        actesArrete.add(TypeActeConstants.TYPE_ACTE_ARRETE_CE);
        return actesArrete;
    }

    private static Set<String> initDecretList() {
        Set<String> actesDecret = new HashSet<>();
    ...
    private static Set<String> initConventionCollective() {
        Set<String> conventionCollective = new HashSet<>();
    ...
    private static Set<String> initReglementaireList() {
        Set<String> nonReglementaire = new HashSet<>();
        nonReglementaire.add(TypeActeConstants.TYPE_ACTE_ACCORD_COLLECTIF_PUBLIC);
    ...
    private static Set<String> initDecretPRList() {
        Set<String> decretPR = new HashSet<>();
    ...
    private static Set<String> initTypeActeVisibleNumeroTexteJOComp() {
        Set<String> inclusion = new HashSet<>();
        ...
        inclusion.add(TypeActeConstants.TYPE_ACTE_ACCORD_COLLECTIF_PUBLIC);

        return inclusion;
    }

    private static Set<String> initTypeActeVisibleCEComp() {
        Set<String> inclusionList = new HashSet<>();
    ...
    @Override
    public boolean isCheminDeCroix(String typeActe) {
        return TYPES_ACTES_CHEMIN_DE_CROIX.contains(typeActe);
    }

    @Override
    public Set<String> getDecretList() {
        return TYPES_ACTES_DECRET;
    }

    @Override
    public Set<String> getOrdonanceList() {
        return TYPES_ACTES_ORDONNANCE;
    }
    ...
    @Override
    public boolean isReglementaire(String typeActe) {
        return TYPES_ACTES_REGLEMENTAIRE.contains(typeActe);
    }
    ...
    public boolean isVisibleTranspositionDirective(String typeActe) {
        return (
            TypeActeConstants.TYPE_ACTE_ORDONNANCE.equals(typeActe) ||
            isDecret(typeActe) ||
            (isArrete(typeActe) && !TypeActeConstants.TYPE_ACTE_ARRETE_CE.equals(typeActe)) ||
            isLoi(typeActe) ||
            isActeTexteNonPublie(typeActe)
        );
    }
    ...
    public List<TypeActe> getAllTypeActe() {
        List<TypeActe> typeActeList = new ArrayList<>();
    ...
    public Map<String, Boolean> getActeVisibility(String typeActe) {
        ...
        if (StringUtils.isNotBlank(typeActe)) {
            ...
            acteVisibility.put(
                ActeVisibilityConstants.TRANSPOSITION_DIRECTIVE,
                isVisibleTranspositionDirective(typeActe)
            );
            acteVisibility.put(
                ActeVisibilityConstants.TRANSPOSITION_ORDONNANCE,
            ...
            acteVisibility.put(
                ActeVisibilityConstants.PUBLICATION_RAPPORT_PRESENTATION,
                isVisiblePublicationRapport(typeActe)
            );

            acteVisibility.put(
                ActeVisibilityConstants.INFORMATIONS_PARLEMENTAIRES,
                isInformationsParlementaires(typeActe)
            );
            acteVisibility.put(ActeVisibilityConstants.RAPPORT_PARLEMENT, isRapportAuParlement(typeActe));
        }
    ...
    private static Set<String> initTypeActeListFDDSaisineRectificative() {
        Set<String> typesActesFDDSaisineRectificative = new HashSet<>();
