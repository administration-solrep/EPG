...
public class FondDeDossierModelServiceImpl implements FondDeDossierModelService {
    ...
    private static final List<String> UNDELETABLE_FOLDERS = Arrays.asList(
        SolonEpgFondDeDossierConstants.FOND_DE_DOSSIER_FOLDER_PATH_RAPPORT_PRESENTATION,
        SolonEpgFondDeDossierConstants.FOND_DE_DOSSIER_FOLDER_PATH_DOC_JOINTE,
        SolonEpgFondDeDossierConstants.FOND_DE_DOSSIER_FOLDER_PATH_EPREUVES,
        SolonEpgFondDeDossierConstants.FOND_DE_DOSSIER_FOLDER_PATH_AVIS_CE
    );
    ...
    @Override
    public DocumentModel getFondDossierModelRoot(final CoreSession session) {
        ...
        if (StringUtils.isEmpty(rootModelFddId)) {
            ...
            if (rootModeleFddList == null || rootModeleFddList.isEmpty()) {
                final String erreur =
                    "Erreur lors de la racine des modèles de fond de dossier : pas de Document trouvé !";
                log.error(erreur);
                throw new NuxeoException(erreur);
            } else if (rootModeleFddList.size() > 1) {
                final String erreur =
                    "Erreur lors de la racine des modèles de fond de dossier : plusieurs document trouvés !";
                log.error(erreur);
                throw new NuxeoException(erreur);
            }
    ...
    public List<String> getFormatsAutorises(final CoreSession session) {
        ...
        try {
            ...
            throw new NuxeoException("Erreur lors de de la récupération de la racine du fond de dossier", e);
        }
    }
    ...
    public DocumentModel updateFondDossierModelRoot(final CoreSession session, final DocumentModel docModel) {
        ...
        try {
            ...
            fddModelRoot.setFormatsAutorises(DossierSolonEpgConstants.FORMAT_AUTORISE_LIST_EXTEND_FOLDER_FOND_DOSSIER);
    ...
    public void createFondDeDossierDefaultRepository(DocumentModel fddModelDoc, final CoreSession session) {
        try {
            ...
            throw new NuxeoException(" erreur lors de la creation de l'arborescence du modele de fond de dossier ", e);
        }
    }
    ...
    @Override
    public List<DocumentModel> getListFondDossierFolderModelFromTypeActe(
        ...
        final List<DocumentModel> docList = QueryUtils.doUFNXQLQueryAndFetchForDocuments(
            ...
            getListFondDossierFolderModelFromTypeActeQuery.toString(),
    ...
    @Override
    public DocumentModel createFondDossierModele(
        ...
        try {
            ...
            final String typeActeLabel = vocService.getLabelFromId(
            ...
            DocumentModel fddModel = session.createDocumentModel(
            ...
            throw new NuxeoException(" erreur lors de la creation d'un modele de fond de dossier ", e);
        }
    }
     ...
     */
    protected DocumentModel createFondDossierModelFolderElement(
        final DocumentModel documentRacine,
        final CoreSession session,
        final String name
    ) {
    ...
    @Override
    public List<FondDeDossierModelNode> getFondDeDossierModelRootNode(
        final CoreSession session,
        ...
        try {
            ...
            return listRepertoire
                .stream()
                .map(this::getFondDeDossierModelNodeFromDocumentModel)
                .collect(Collectors.toList());
        } catch (final NuxeoException e) {
            throw new NuxeoException(
                "erreur lors de la recuperation des repertoire racine du modele de fond de dossier",
                e
            );
        }
    }
    ...
    private FondDeDossierModelNode getFondDeDossierModelNodeFromDocumentModel(final DocumentModel docModel) {
        ...
        // supprimer.
        int segmentToRemove = Math.max(segmentCount - 2, 0);
        path = path.removeFirstSegments(segmentToRemove);
        segmentCount = path.segmentCount();

        if (segmentCount == 1) {
            node.setType(FondDeDossierModelType.FOND_DOSSIER_FOLDER_UNDELETABLE_ROOT);
        } else if (UNDELETABLE_FOLDERS.contains(path.toString())) {
    ...
    @Override
    public List<FondDeDossierModelNode> getChildrenFondDeDossierModelNode(
        final String documentId,
        final CoreSession session
    ) {
        // initialisation des variables
        final List<FondDeDossierModelNode> subGroupsNodeList = new ArrayList<>();
        ...
        try {
            ...
            throw new NuxeoException(
                "erreur lors de la recuperation de repertoire fils du modele de fond de dossier",
                e
            );
        }
    ...
    public void createNewDefaultFolderInTree(
        ...
        treeService.createNewDefaultFolderInTree(
            documentParent,
            session,
            SolonEpgFondDeDossierConstants.FOND_DE_DOSSIER_MODEL_FOLDER_DOCUMENT_TYPE,
            null,
            typeActe,
            name
        );
    ...
    public void createNewFolderNodeBefore(
        ...
        // appel au service générique
        final TreeService treeService = SolonEpgServiceLocator.getTreeService();
        treeService.createNewFolderNodeBefore(
            documentCourant,
            session,
            SolonEpgFondDeDossierConstants.FOND_DE_DOSSIER_MODEL_FOLDER_DOCUMENT_TYPE,
            null,
    ...
    public void createNewFolderNodeAfter(
        ...
        // appel au service générique
        final TreeService treeService = SolonEpgServiceLocator.getTreeService();
        treeService.createNewFolderNodeAfter(
            documentCourant,
            session,
            SolonEpgFondDeDossierConstants.FOND_DE_DOSSIER_MODEL_FOLDER_DOCUMENT_TYPE,
            null,
    ...
    @Override
    public DocumentModel editFolder(
        final DocumentModel documentCourant,
        final CoreSession session,
        final String name,
        final String typeActe
    ) {
        FondDeDossierModelNode node = getFondDeDossierModelNodeFromDocumentModel(documentCourant);
        if (node.getType() != FondDeDossierModelType.FOND_DOSSIER_FOLDER_DELETABLE) {
            throw new NuxeoException("Ce dossier ne peux pas être édité");
        }
    ...
    public void deleteFolder(final DocumentModel documentCourant, final CoreSession session, final String typeActe) {
        FondDeDossierModelNode node = getFondDeDossierModelNodeFromDocumentModel(documentCourant);
        if (node.getType() != FondDeDossierModelType.FOND_DOSSIER_FOLDER_DELETABLE) {
            throw new NuxeoException("Ce dossier ne peux pas être supprimé");
        }
