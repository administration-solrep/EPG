 ...
 *
 * @author Adeline Hullot
 */
public class OneStatGenerationBatchListener extends AbstractBatchEventListener {
    private static final STLogger LOGGER = STLogFactory.getLog(OneStatGenerationBatchListener.class);

    public OneStatGenerationBatchListener() {
        super(LOGGER, SolonEpgEventConstant.BATCH_EVENT_STAT_ONE_GENERATION);
    }
    ...
    protected void processEvent(CoreSession session, Event event) {
        ...
        try {
            ...
            String idRapport = (String) event.getContext().getProperty("idRapport");
            if (StringUtils.isNotEmpty(idRapport)) {
                ANReportEnum aNReport = ANReportEnum.valueOf(idRapport);
        ...
        try {
            suiviBatchService.createBatchResultFor(
                batchLoggerModel,
                "Generation de resultat d'une statistique avec birt",
                endTime - startTime
            );
        } catch (Throwable e) {
    ...
    private Map<String, Serializable> assignParameters(
        ...
        if (curLegis) {
            ...
            if (lastIndex >= 0) {
                ...
                inputValues.put(
                    "LEGISLATURES_LABEL",
                    "la " + paramAn.getLegislatures().get(lastIndex) + " législature"
                );
            }
        }
