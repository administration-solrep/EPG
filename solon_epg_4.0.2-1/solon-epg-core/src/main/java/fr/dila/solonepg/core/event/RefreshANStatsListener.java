...
public class RefreshANStatsListener implements PostCommitEventListener {
        ...
        try (CloseableCoreSession session = SessionUtil.openSession()) {
            if (session != null) {
                try {
                    LOGGER.info(
                        session,
                        SSLogEnumImpl.CREATE_BIRT_TEC,
                        "Rafraichissement du rapport : " + report.getName()
                    );
                    ...
                    try {
                        sendMailRefreshOK(
                            session,
                            user,
                            getLibelle(session, report, inputValues),
                            anStatsService.getHorodatageRequest(session, report, user)
                        );
    ...
    private String getLibelle(CoreSession session, ANReport report, Map<String, Serializable> inputValues) {
        StringBuilder libelleStat = new StringBuilder("\"" + report.getType().getLibelle() + "\"");
        String ministereLabel = (String) inputValues.get("MINISTEREPILOTELABEL_PARAM");
        String idDossier = (String) inputValues.get("DOSSIERID_PARAM");
        if (ministereLabel != null) {
            libelleStat.append(" pour le ministère \"").append(ministereLabel).append("\"");
        }
        ...
        if (idDossier != null) {
            ...
            try {
                ...
                if (STConstant.DOSSIER_DOCUMENT_TYPE.equals(doc.getType())) {
                    ...
                    ActiviteNormativeConstants.ACTIVITE_NORMATIVE_DOCUMENT_TYPE.equalsIgnoreCase(doc.getType())
                ) {
                ...
                libelleStat.append(" pour le dossier \"").append(norDossier).append("\"");
            } catch (NuxeoException exc) {
    ...
    private void sendMailRefreshOK(CoreSession session, String user, String reportName, String dateRequest) {
        ...
        String corpsTemplate =
            "Bonjour, le rafraichissement de la statistique " +
            reportName +
            " demandé le " +
            dateRequest +
            " est terminé.";
    ...
    private void sendMailRefreshKO(
        CoreSession session,
        String user,
        String reportName,
        String dateRequest,
