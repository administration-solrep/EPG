...
public class TraitementPapierServiceImpl extends LogDocumentUpdateServiceImpl implements TraitementPapierService {
    ...
    protected void fireEvent(
        ...
        String traitementPapierLabel = vocService.getLabelFromId(
            STVocabularyConstants.TRAITEMENT_PAPIER_LABEL,
            entryKey.substring(3),
     ...
     * @return
     */
    protected Object getVocabularyValue(
        CoreSession session,
        String entryKey,
        Object id,
    ...
    @Override
    public void saveTraitementPapier(CoreSession session, DocumentModel dossierDoc) {
    ...
    private void updateChargeMissionAndConseillerPmValidation(CoreSession session, DocumentModel dossierDoc) {
        ...
        Set<String> chargeMission = new HashSet<>(dossier.getNomCompletChargesMissions());

        List<DestinataireCommunication> listDestinataireCommunicationCM = traitementPapier.getChargeMissionCommunication();
        ...
        dossier.setNomCompletChargesMissions(new ArrayList<>(chargeMission));
        dossier.save(session);
        session.save();

        Set<String> conseillerPm = new HashSet<>(dossier.getNomCompletConseillersPms());
        ...
        dossier.setNomCompletConseillersPms(new ArrayList<>(conseillerPm));
        dossier.save(session);
        session.save();
    }
     ...
     * @return
     */
    private Boolean isAvisFavorable(String avis) {
        ...
        try {
            ...
            String type = (String) documentModel.getProperty(
            ...
            if (session != null) {
                try {
                    ...
                    LOGGER.error(STLogEnumImpl.DEFAULT, "Error while closing session");
                }
            }
        }
    }
