...
public class IndexationEpgServiceImpl implements IndexationEpgService {
    ...
    public static final String FIND_ALL_INDEXATION_RUBRIQUE =
        "SELECT * FROM IndexationRubrique  WHERE ecm:isProxy = 0 ORDER BY dc:title, ecm:uuid";
     ...
     */
    private static final STLogger LOGGER = STLogFactory.getLog(IndexationEpgServiceImpl.class);

    private enum TypeIndexation {
        RUBRIQUE("IndexationRubrique", "indexation_rubrique_solon_epg"),
        MOT_CLE("IndexationMotCle", "indexation_mot_cle_solon_epg");
    ...
    @Override
    public DocumentModelList findAllIndexationRubrique(final CoreSession session) {
        return new UnrestrictedQueryRunner(
            session,
            FIND_ALL_INDEXATION_RUBRIQUE,
    ...
    @Override
    public DocumentModelList findAllIndexationMotCle(final CoreSession session) {
        ...
        if (principal != null && !principal.isMemberOf(SolonEpgBaseFunctionConstant.ADMINISTRATION_BULLETIN_READER)) {
            final Set<String> ministereSet = new HashSet<>();
    ...
    @Override
    public void createIndexationRubrique(final CoreSession session, final String intitule) {
        ...
        final DocumentModel modelDesired = session.createDocumentModel(
            SolonEpgIndextionConstants.INDEXATION_PATH,
            UUID.randomUUID().toString(),
            SolonEpgIndextionConstants.INDEXATION_RUBRIQUE_DOCUMENT_TYPE
        );
    ...
    private void checkIndexationCount(final CoreSession session, final TypeIndexation type, final String intitule) {
        ...
        try {
            final Long result = QueryUtils.doCountQuery(
                session,
    ...
    @Override
    public DocumentModel createIndexationMotCle(
        final CoreSession session,
        final String intitule,
        final List<String> ministereIds
    ) {
        ...
        final DocumentModel modelDesired = session.createDocumentModel(
            SolonEpgIndextionConstants.INDEXATION_PATH,
            UUID.randomUUID().toString(),
            SolonEpgIndextionConstants.INDEXATION_MOT_CLE_DOCUMENT_TYPE
        );
    ...
    private void updateIndexation(final CoreSession session, final DocumentModel doc) {
        session.saveDocument(doc);
        session.save();
    }

    private void removeIndexation(final CoreSession session, final DocumentModel doc) {
    ...
    @Override
    public List<EntiteNode> findAllMinistereForIndexation(final CoreSession session) {
        final SSPrincipal principal = (SSPrincipal) session.getPrincipal();
        final List<EntiteNode> nodes = new ArrayList<>();
        final STMinisteresService ministeresService = STServiceLocator.getSTMinisteresService();
        ...
        if (!principal.isMemberOf(SolonEpgBaseFunctionConstant.ADMINISTRATION_INDEXATION_UPDATER)) {
            ...
            List<OrganigrammeNode> currentGouvChildren = organigrammeService.getChildrenList(
                session,
                currentGouv,
                Boolean.TRUE
            );
    ...
    @Override
    public List<String> findAllIndexationMotCleForDossier(
        final CoreSession session,
    ...
    public void migrerGestionIndexation(
        final CoreSession session,
        final String ancienMinistereId,
        final String nouveauMinistereId,
        final MigrationLoggerModel migrationLoggerModel
    ) {
        migrationLoggerModel.setMotsCles(0);
        final SSChangementGouvernementService chgtGouvService = SolonEpgServiceLocator.getChangementGouvernementService();
        chgtGouvService.flushMigrationLogger(migrationLoggerModel);

        // récupération de toutes les indexations de l'ancien ministère
        final List<DocumentModel> indexationToRename = getIndexationMotCleQueryFromIdMinistere(
            session,
            ancienMinistereId
        );
        if (indexationToRename == null) {
            ...
            final MigrationDetailModel migrationDetailModel = chgtGouvService.createMigrationDetailFor(
                ...
                MigrationDiscriminatorConstants.MOTSCLES,
                "Aucun mots-clés trouvé"
            );
            ...
            for (final DocumentModel documentModel : indexationToRename) {
                ...
                final MigrationDetailModel migrationDetailModel = chgtGouvService.createMigrationDetailFor(
                    migrationLoggerModel,
                    MigrationDiscriminatorConstants.MOTSCLES,
                    "Migration du mots-clés " + indexationMotsCles.getIntitule()
                );
    ...
    @Override
    public List<DocumentModel> getIndexationMotCleQueryFromIdMinistere(
        final CoreSession session,
        ...
        return QueryUtils.doUFNXQLQueryAndFetchForDocuments(
            session,
            IndexationMotCle.class.getSimpleName(),
            query.toString(),
            params.toArray()
        );
    }
