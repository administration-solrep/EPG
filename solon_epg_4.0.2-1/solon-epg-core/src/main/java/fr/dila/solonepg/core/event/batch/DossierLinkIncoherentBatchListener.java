...
public class DossierLinkIncoherentBatchListener extends AbstractDossierLinkIncoherentBatchListener {
    ...
    private static final String STEP_NOT_RUNNING_QUERY =
        "select id from dossier_solon_epg where lastdocumentroute in (select documentrouteid from routing_task " +
    ...
    private static final String DOSSIER_LINK_EXISTING_QUERY =
        "select id from dossier_solon_epg where lastdocumentroute in (select id from misc where lifecyclestate ='done') " +
    ...
    protected DataSource generateData(CoreSession session, List<String> dossiersIds) {
        return EpgExcelUtil.creationListDossierExcelFromIds(session, dossiersIds);
    }
