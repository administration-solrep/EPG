 ...
 */
@Operation(
    id = GenererLotExtractionOperation.ID,
    label = "Vérification éligibilité et création de lot pour archivage",
    description = GenererLotExtractionOperation.DESCRIPTION
)
public class GenererLotExtractionOperation {
    ...
    public static final String DESCRIPTION =
        "Cette opération vérifie l'éligibilité des dossiers à l'archivage et lance la procédure de création de lot";
    ...
    public void run() throws Exception {
        ...
        if (forceGenDosArchive) {
            LOGGER.info(
                STLogEnumImpl.LOG_INFO_TEC,
                "Mode force activé, tous les dossiers doivent être traités, incluant les dossiers au statut archivé"
            );
        }
        ...
        if (!docList.isEmpty()) {
            ...
            List<DocumentModel> docListPourLot = new ArrayList<>();
                ...
                if (
                    ...
                    (
                        ...
                        (
                            ...
                            !dossier
                                .getStatutArchivage()
                                .equals(VocabularyConstants.STATUT_ARCHIVAGE_EXTRACTION_EN_COURS) &&
            ...
            ARCHIVAGE_LOGGER.info(
                "Création d'un lot contenant les " +
                docListPourLot.size() +
                " dossier(s) suivant(s) : " +
                listNorPourLot.toString()
            );

            ExtractionLotService extractionLotService = SolonEpgAdamantServiceLocator.getExtractionLotService();
            extractionLotService.createAndAssignLotToDocumentsByIds(
                docListPourLot.stream().map(DocumentModel::getId).collect(Collectors.toList())
            );
        }
    }
