 ...
 */
@Operation(
    id = RepriseFicheLoi.ID,
    category = CaseConstants.CASE_MANAGEMENT_OPERATION_CATEGORY,
    label = RepriseFicheLoi.LABEL,
    description = RepriseFicheLoi.DESCRIPTION
)
public class RepriseFicheLoi {
    public static final String ID = "SolonEpg.Reprise.RepriseFichesLoi";

    public static final String LABEL = "Reprise des fiches lois sans nature de MGPP";

    public static final String DESCRIPTION =
        "Cette opération reprend les données de nature de loi présentes dans EPP pour les insérer dans les fiches loi de MGPP";
    ...
    public void reprise() {
        try {
            ...
            LOGGER.debug(
                ...
                "On s'apprète à traiter " + lstFiches.size() + " fiches loi"
            );
            int nbDone = 0;
            int nbNotFound = 0;
            ...
            LOGGER.debug(
                session,
                STLogEnumImpl.LOG_DEBUG_TEC,
                "Le traitement est terminé et a traité " + nbDone + " et ignoré " + nbNotFound + " fiches loi"
            );
        } catch (Exception e) {
            LOGGER.error(
                session,
                STLogEnumImpl.FAIL_UPDATE_DOSSIER_TEC,
                "Une erreur est survenue lors du traitement de la reprise",
                e
            );
        }
    }

    private List<FicheLoi> getFichesLoiSansNature() {
        return SolonMgppServiceLocator.getDossierService().findFicheLoiWithoutNature(session);
    }

    private NatureLoi findNature(String idDossier) throws WSProxyFactoryException, Exception {
        ...
        RechercherEvenementResponse response = SolonMgppServiceLocator
            .getRechercheService()
            .findMessageWS(criteres, session);
