...
public class PurgeCalendrierBatchListener extends AbstractBatchEventListener {
    ...
    protected void processEvent(CoreSession session, Event event) {
        ...
        try {
            ...
            long delai =
                (parametrageMgpp.getDelaiPurgeCalendrier() == null ? 1 : parametrageMgpp.getDelaiPurgeCalendrier());

            final Calendar purgeCalendar = DateUtil.removeMonthsToNow((int) delai);
        ...
        try {
            STServiceLocator
                ...
                .createBatchResultFor(
                    batchLoggerModel,
                    "Exécution du Batch de purge du calendrier",
                    endTime - startTime
                );
        } catch (Exception e) {
