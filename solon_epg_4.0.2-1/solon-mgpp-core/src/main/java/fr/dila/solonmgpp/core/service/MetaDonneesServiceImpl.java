...
public class MetaDonneesServiceImpl extends DefaultComponent implements MetaDonneesService {
    ...
    static {
        ...
        for (EtatMessage etatMessage : EtatMessage.values()) {
            if (
                EtatMessage.TRAITE.equals(etatMessage) ||
                EtatMessage.EMIS.equals(etatMessage) ||
                EtatMessage.AR_RECU.equals(etatMessage)
            ) {
    ...
    public MetaDonneesDescriptor getEvenementType(final String evenementType) {
        ...
        if (metaDonneesDescriptor == null) {
            throw new NuxeoException("Type d'événement inconnu : " + evenementType);
        }
