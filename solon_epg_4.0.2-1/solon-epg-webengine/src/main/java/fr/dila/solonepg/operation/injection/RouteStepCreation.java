...
@Operation(
    id = RouteStepCreation.ID,
    category = Constants.CAT_DOCUMENT,
    label = "Create Route Step",
    description = "Create Route Step."
)
public class RouteStepCreation {
    ...
    public DocumentModel run(final DocumentModel doc) throws Exception {
        ...
        try {
            ...
            if (destinataire.startsWith("uniteStructurelle-")) {
                ...
                final List<EntiteNode> nouveauMinistereList = ministeresService.getMinistereParentFromUniteStructurelle(
                    identifier
                );
                ...
                if (posteDoc == null && !"validated".equals(statut) && !"validatedAndFdrDone".equals(statut)) {
                    posteDoc =
                        SolonEpgServiceLocator
                            ...
                            .createDesactivatePoste(session, destinataire, destinataireLabel);
                    poste = destinataire;
                } else {
                    poste = destinataire;
                }
            ...
            try {
                if ("validated".equals(statut) || "validatedAndFdrDone".equals(statut)) {
                    ...
                    if (step.getParentRef() != null) {
                        ...
                        if (
                            FeuilleRouteConstant.TYPE_FEUILLE_ROUTE_STEP_FOLDER.equals(type) ||
                    ...
                    final Mailbox userMailbox = SSServiceLocator
                        .getMailboxPosteService()
                        .getOrCreateMailboxPoste(session, poste);
                    step.setPropertyValue(
                        CaseDistribConstants.STEP_DISTRIBUTION_MAILBOX_ID_PROPERTY_NAME,
                        userMailbox.getId()
                    );
    ...
    private Set<String> getMinistereSet(final String userId) throws DirectoryException {
        ...
        final STMinisteresService ministeresService = STServiceLocator.getSTMinisteresService();
        final List<String> posteNameList = PropertyUtil.getStringListProperty(
            userModel,
