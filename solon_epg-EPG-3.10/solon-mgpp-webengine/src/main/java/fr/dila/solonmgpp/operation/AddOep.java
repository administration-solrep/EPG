...
public class AddOep {
    ...
    public void run() throws Exception {
        ...
        try {
            if (StringUtils.isNotBlank(idCommun)) {
                ...
                FichePresentationOEP fiche = SolonMgppServiceLocator.getDossierService().findOrCreateFicheRepresentationOEP(session, evenementDTO);
                ...
                SolonMgppServiceLocator.getDossierService().createFicheRepresentationOEP(session, fiche.getDocument(),
                        new ArrayList<DocumentModel>(), new ArrayList<DocumentModel>());

                LOGGER.debug(session, MgppLogEnumImpl.ADD_OEP_PROCESS_TEC,"Add Oep OK - idCommun = " + idCommun) ;
                
                LOGGER.debug(session, MgppLogEnumImpl.ADD_OEP_PROCESS_TEC,"Add Oep publication communication - idCommun = " + idCommun) ;

                // reload fiche
                FichePresentationOEP fichePresentationOEP = SolonMgppServiceLocator.getDossierService().findFicheOEP(session, idDossier);

                try {
                    // publication de 2 EVT49 a l'injection d'un oep
                    publierEVT49(fichePresentationOEP, fileName, content);
                } catch (Exception e) {
                    LOGGER.debug(session, MgppLogEnumImpl.FAIL_ADD_OEP_PROCESS_TEC,"Add Oep publication communication - idCommun = " + idCommun, e) ;
                }
    ...
    private void publierEVT49(FichePresentationOEP fichePresentationOEP, String fileName, byte[] content) throws Exception {
        ...
        else if(rechercherEvenementResponse.getStatut()==null || !TraitementStatut.OK.equals(rechercherEvenementResponse.getStatut())) {            
            ClientException clientException =   new ClientException("Erreur de communication avec SOLON EPP, la récupération des messages a échoué."
                    + WSErrorHelper.buildCleanMessage(rechercherEvenementResponse.getMessageErreur()));            
            LOGGER.error(session, MgppLogEnumImpl.FAIL_GET_WS_TEC, clientException);
        ...
        for (Message message : rechercherEvenementResponse.getMessage()) {
            if (last == null
                    ...
                    && DateUtil.xmlGregorianCalendarToDate(last.getDateEvenement()).before(
        ...
        List<DocumentModel> listRepresentantAN = dossierService.fetchRepresentantOEP(session, VocabularyConstants.REPRESENTANT_TYPE_AN,
        ...
        if (creerVersionResponseAN == null) {
            throw new ClientException("Erreur de communication avec SOLON EPP, la creation de la communication a échoué.");
        }
        else if(creerVersionResponseAN.getStatut()==null || !TraitementStatut.OK.equals(creerVersionResponseAN.getStatut())) {
            
          ClientException clientExc =  new ClientException("Erreur de communication avec SOLON EPP, la creation de la communication a échoué."
        ...
        List<DocumentModel> listRepresentantSE = dossierService.fetchRepresentantOEP(session, VocabularyConstants.REPRESENTANT_TYPE_SE,
                fichePresentationOEP.getDocument().getId());
        ...
        else if(creerVersionResponseSE.getStatut()==null || !TraitementStatut.OK.equals(creerVersionResponseSE.getStatut())) {            
            ClientException clientExc =  new ClientException("Erreur de communication avec SOLON EPP, la creation de la communication a échoué."
