...
then 
	...
	echo " "
	echo "Exemple : sh ldap-genererLdifEnvironnement.sh 'localhost' 'cn=nuxeo5,ou=SolonEpg,dc=dila,dc=fr' 'changeme' 'ldapscript.ldif' 'suffixe'"
	exit 1
else 
	echo "Hote LDAP : "$1
	echo "Compte LDAP : "$2
...
invariants=("uid=ws_ce,ou=people,ou=SolonEpg,dc=dila,dc=fr"
	"uid=ws_dila,ou=people,ou=SolonEpg,dc=dila,dc=fr"
	"uid=ws_fdj_promulgation,ou=people,ou=SolonEpg,dc=dila,dc=fr"
	"uid=ws_infopar,ou=people,ou=SolonEpg,dc=dila,dc=fr"
	"uid=ws_legho,ou=people,ou=SolonEpg,dc=dila,dc=fr"
	"uid=com_epp,ou=people,ou=SolonEpg,dc=dila,dc=fr"
	"uid=hebergementDILA,ou=people,ou=SolonEpg,dc=dila,dc=fr")
...
admins=($(ldapsearch -h ${hote} -D ${compte} -w ${mdp} -b "cn=Administrateur fonctionnel,ou=groups,ou=SolonEpg,dc=dila,dc=fr" -s sub -LLL uniqueMember | perl -p00e 's/\r?\n //g' | grep uniqueMember | grep ou=people | perl -p00e 's/uniqueMember:(.*)/$1/g'))
...
SAVEIFS=$IFS
IFS=$'\n'
allusers=($(ldapsearch -h ${hote} -D ${compte} -w ${mdp} -b "ou=SolonEpg,dc=dila,dc=fr" -s sub "objectclass=personne" dn -LLL | grep dn | perl -p00e 's/(.*):(.*)/$2/g'))
