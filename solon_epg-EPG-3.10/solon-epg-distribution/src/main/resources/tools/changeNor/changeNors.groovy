 ...
 * Le changement par rapport à reparePublication est la modification du ministère de rattachement.
 * Le dossier garde l'ancien nom - voir pour changer cela.
 *
 */
...
class DossierUtils{
	...
	static void rattacherDossier(session,dossier,correspond){
		if (dossier !=null){
			dossier.setMinistereResp(correspond.newMinistere.id);
			dossier.setMinistereAttache(correspond.newMinistere.id);
			String newNumero = getNewNumero(dossier.getNumeroNor(),correspond.newMinistere);
			dossier.setNumeroNor(newNumero);
			dossier.setTitle(newNumero);
	...
	public static rattacherTous(session,set,nor_file){
		new File(nor_file).splitEachLine(" ") { fields ->
			...
			if (correspond == null){
				...
				String newNor = getNewNumero(nor,correspond.newMinistere);
				//print "Traitement du nor : " + nor;
				Dossier dossier = getDossierFromBdd(session,nor);
				if (dossier != null){
					print "Rattache le dossier " + nor + " vers nor " + newNor + "et le ministere id = " + correspond.newMinistere.id +"; trig = " + correspond.newMinistere.trigramme;
					rattacherDossier(session,dossier,correspond);
				}
				else{
					print "Dossier " + nor + " non trouvé";
				}
			}
		}
	}
...
String CORRESPONDANCE_MINS = "/tmp/mapOldMinToNewMin.txt";
