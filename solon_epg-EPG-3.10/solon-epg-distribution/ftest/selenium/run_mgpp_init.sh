...
HIDE_FF=${HIDE_FF:-}
SUITES=${SUITES:-"suite"}
URL=${URL:-http://localhost:8180/solon-epg/}
...
sed "s,/path/to/project-ear/ftest/selenium,$PWD/data/,g" < $USER_EXTENSIONS.sample > $USER_EXTENSIONS
...
for suite in $SUITES; do
    ...
    $CMD "$PWD/tests/mgpp/init-$suite.html" "$PWD/result-init-$suite.html" $CMD_END
    if [ $? != 0 ]; then
