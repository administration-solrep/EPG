...
public class InjectionEPPListener implements PostCommitEventListener {
	...
	public void handleEvent(EventBundle events) throws ClientException {
		...
		if(event !=null) {
			...
			StringBuilder injectionError = (StringBuilder) ctx.getProperties().get(SolonMgppEventConstants.INJECTION_ERROR);
			...
			try {
				...
				List<InjectionGvtDTO> listInjection = (List<InjectionGvtDTO>) ctx.getProperties().get(
						SolonMgppEventConstants.LIST_INJECTION_EPP_PARAM);
				@SuppressWarnings("unchecked")
				List<InjectionEpgGvtDTO> listCompared = (List<InjectionEpgGvtDTO>) ctx.getProperties().get(
						SolonMgppEventConstants.LIST_COMPARAISON_EPP_PARAM);
				SolonMgppServiceLocator.getMGPPInjectionGouvernementService().createComparedDTO(session, listInjection,
						listCompared);
				LOGGER.info(session, STLogEnumImpl.PROCESS_INJECTION_GOUVERNEMENT_TEC,
