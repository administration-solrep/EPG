...
public class NotificationServiceImpl implements NotificationService {
    ...
    private static final String SELECT_N_UID_FROM = STQueryConstant.SELECT + " n.ecm:uuid as id " + STQueryConstant.FROM;
    private static final String AS_N_WHERE = STQueryConstant.AS + " n " + STQueryConstant.WHERE;

    private static final String QUERY_NOTIF_ORDER_BY_DATE_ARRIVEE = SELECT_N_UID_FROM + //
            SolonMgppNotificationConstants.NOTIFICATION_DOC_TYPE + AS_N_WHERE  + //
            TYPE_NOTIF + " != ? " + " ORDER BY " + DATE_ARRIVEE + " DESC ";

    private static final String QUERY_NOTIFICATIONS_CORBEILLE = SELECT_N_UID_FROM + //
            SolonMgppNotificationConstants.NOTIFICATION_DOC_TYPE + AS_N_WHERE + //
            MESSAGE_CORBEILLE_LIST + " = ? AND " + DATE_ARRIVEE + " > ? AND " + TYPE_NOTIF + " is null ";

    private static final String QUERY_NOTIFICATIONS_EVENEMENT = SELECT_N_UID_FROM + //
            SolonMgppNotificationConstants.NOTIFICATION_DOC_TYPE + AS_N_WHERE + //
            EVENEMENT_ID + " = ? AND " + DATE_ARRIVEE + " > ? AND " + TYPE_NOTIF + " != ? ";

    private static final String PATH_TO_NOTIF_TDR = SolonMgppNotificationConstants.NOTIFICATION_ROOT_PATH + "/notificationTDR";

    private static final String MERGE_MGPP_INFO_CORB = "MERGE INTO " + SolonMgppCorbeilleConstant.MGPP_INFO_CORBEILLE_TABLE_NAME
    		+ " USING dual ON (" + SolonMgppCorbeilleConstant.COL_CORBEILLE_MGPP_INFO_CORBEILLE + " = :corbeilleId)"
    		+ " WHEN NOT MATCHED THEN INSERT (" + SolonMgppCorbeilleConstant.COL_CORBEILLE_MGPP_INFO_CORBEILLE + "," + SolonMgppCorbeilleConstant.COL_HASMESS_MGPP_INFO_CORBEILLE
    		+ ") VALUES (:corbeilleId, :hasMess) WHEN MATCHED THEN UPDATE SET " + SolonMgppCorbeilleConstant.COL_HASMESS_MGPP_INFO_CORBEILLE + " = :hasMess";
     ...
     */
    @Override
    public NotificationDoc createNotication(final CoreSession session, final Notifications notifications) throws ClientException {
     ...
     * @throws ClientException
     */
    private DocumentModel createBareNotificationDoc(final CoreSession session) throws ClientException {
        DocumentModel notificationDocModel = new DocumentModelImpl(SolonMgppNotificationConstants.NOTIFICATION_ROOT_PATH, "nt",
     ...
     */
    @Override
    public Long getCountNotificationsCorbeilleSince(final CoreSession session, final String corbeille, final Calendar date) throws ClientException {

        final List<Object> paramList = new ArrayList<Object>();
        paramList.add(corbeille);
        paramList.add(date);

        return QueryUtils.doCountQuery(session, FlexibleQueryMaker.KeyCode.UFXNQL.key + QUERY_NOTIFICATIONS_CORBEILLE, paramList.toArray());
    }

    @Override
    public Long getCountNotificationsEvenementSince(final CoreSession session, final String evenementId, final Calendar date) throws ClientException {
        ...
        return QueryUtils.doCountQuery(session, FlexibleQueryMaker.KeyCode.UFXNQL.key + QUERY_NOTIFICATIONS_EVENEMENT, paramList.toArray());
    }

    @Override
    public Calendar getLastNotificationDate(final CoreSession session) throws ClientException {

        final List<Object> paramList = new ArrayList<Object>();
        paramList.add(SolonMgppNotificationConstants.NOTIFICATION_TDR);
        final List<DocumentModel> docs = QueryUtils.doUFNXQLQueryAndFetchForDocuments(session, SolonMgppNotificationConstants.NOTIFICATION_DOC_TYPE,
                QUERY_NOTIF_ORDER_BY_DATE_ARRIVEE, paramList.toArray(), 1, 0);
    ...
    @Override
    public void notifyMAJTableDeReferenceEPP(final CoreSession session) throws ClientException {
    ...
    @Override
    public Calendar getLastNotificationUpdateCache(final CoreSession session) throws ClientException {
        final DocumentModel notificationDocDoc = getOrCreateNotificationTDR(session);
        if (notificationDocDoc == null) {
     ...
     * @throws ClientException 
     */
    private DocumentModel getOrCreateNotificationTDR(final CoreSession session) throws ClientException {
        if (session == null) {
        	return null;
        }
        ...
        if (session.exists(pathToNotif)) {
        	...
        	notificationDocDoc = new DocumentModelImpl(SolonMgppNotificationConstants.NOTIFICATION_ROOT_PATH, "notificationTDR",
                    SolonMgppNotificationConstants.NOTIFICATION_DOC_TYPE);
    ...
    @Override 
    public void updateMgppInfoCorbeille(CoreSession session) throws ClientException {
    	WSEpp wsEpp = getWsEpp(session);
        HasCommunicationNonTraiteesResponse response = getHasCommunicationNonTraiteesResponse(session, wsEpp, new HasCommunicationNonTraiteesRequest());
     ...
     * @throws ClientException 
     */
    private void refreshMgppInfoCorbeille(CoreSession session, List<String> corbeilleList) throws ClientException {
    	WSEpp wsEpp = getWsEpp(session);
    ...
    private WSEpp getWsEpp(CoreSession session) throws ClientException {
    	WSEpp wsEpp = null;
        try {
    ...
    private HasCommunicationNonTraiteesResponse getHasCommunicationNonTraiteesResponse(CoreSession session, 
    		WSEpp wsEpp, HasCommunicationNonTraiteesRequest request) throws ClientException {
        ...
        if (response == null) {
            throw new ClientException("Erreur de communication avec SOLON EPP, la récupération des informations des corbeilles a échoué.");
            ...
            throw new ClientException("Erreur de communication avec SOLON EPP, la récupération des corbeilles a échoué." + WSErrorHelper.buildCleanMessage(response.getMessageErreur()));
        }
        return response;
    }
     ...
     * @throws ClientException 
     */
    private void updateMgppInfoCorbeilleWithData(CoreSession session, final List<CorbeilleInfos> corbeillesInfosList) throws ClientException {    	
    ...
    private static void activatePersistenceProvider() {
        ...
        try {
            ...
            persistenceProvider = persistenceProviderFactory.newProvider("SWCORBEILLEINFOS");
