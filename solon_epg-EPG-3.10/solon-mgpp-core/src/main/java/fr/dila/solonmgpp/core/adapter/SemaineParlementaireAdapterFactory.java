package fr.dila.solonmgpp.core.adapter;

import org.nuxeo.ecm.core.api.DocumentModel;
import org.nuxeo.ecm.core.api.adapter.DocumentAdapterFactory;

import fr.dila.cm.exception.CaseManagementRuntimeException;
import fr.dila.solonmgpp.api.domain.SemaineParlementaire;
import fr.dila.solonmgpp.core.domain.SemaineParlementaireImpl;

public class SemaineParlementaireAdapterFactory implements DocumentAdapterFactory {

	@Override
	public Object getAdapter(DocumentModel doc, Class<?> arg1) {

		if (!SemaineParlementaire.SEMAINE_PARLEMENTAIRE_DOC_TYPE.equals(doc.getType())) {
			throw new CaseManagementRuntimeException("Document should be of type "
					+ SemaineParlementaire.SEMAINE_PARLEMENTAIRE_DOC_TYPE);
		}

		return new SemaineParlementaireImpl(doc);
	}
}
