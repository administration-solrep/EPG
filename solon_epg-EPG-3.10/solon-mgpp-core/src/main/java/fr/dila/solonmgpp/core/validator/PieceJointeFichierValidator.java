...
public class PieceJointeFichierValidator {
     ...
     */
    public static final Pattern PIECE_JOINTE_FICHIER_FILENAME_PATTERN = Pattern.compile("^(?!^(PRN|AUX|CLOCK\\$|NUL|CON|COM\\d|LPT\\d|\\..*)(\\..+)?$)[^\\x00-\\x1f\\\\?*:\\\";|/]+$");
    ...
    public void validatePieceJointeFichierFileName(String filename) throws ClientException {
        ...
        if (!matcher.matches()) {
            throw new ClientException("Le nom de fichier + '" + filename + "' doit comporter uniquement les caractères suivants (accentués ou non) : [A-Z a-z 0-9], tiret (-), souligné (_) et espace");
        }
    }
}
