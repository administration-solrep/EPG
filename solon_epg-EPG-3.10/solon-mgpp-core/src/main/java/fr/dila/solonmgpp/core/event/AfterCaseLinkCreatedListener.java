...
public class AfterCaseLinkCreatedListener implements PostCommitEventListener {
            ...
            if (StringUtils.isNotBlank(typeActe)) {
                ...
                if (listLoi.contains(typeActe)) {
                    if (StringUtils.isBlank(dossier.getIdDossier())) {
                        ...
                        SolonMgppServiceLocator.getEvenementService().createEvenementEppEvt01Brouillon(dossier, eventCtx.getCoreSession());
                    }
                    ...
                    SolonMgppServiceLocator.getEvenementService().createEvenementEppEvt44Brouillon(dossier, eventCtx.getCoreSession());
                } else if (acteService.isDecretPR(typeActe)) {
                    // Décret du Président de la République
                    SolonMgppServiceLocator.getDossierService().notifierDecret(dossier.getNumeroNor(), eventCtx.getCoreSession(), Boolean.TRUE);
                }
            }
        }
    ...
    private String retrieveRouteStepType(final CoreSession session, final String routeStepDocId) throws ClientException {
    ...
    private Dossier retrieveDossier(final CoreSession session, final String dossierDocId) throws ClientException {
