...
public class FicheSignaletiqueLoiDTO extends AbstractFicheSignaletiqueApplicationDTO implements Serializable {
	...
	public FicheSignaletiqueLoiDTO(ActiviteNormative activiteNormative, CoreSession session) throws ClientException {
	...
	private void computeNatureTexte(TexteMaitre texteMaitre) {
		if (texteMaitre.getProcedureVote() != null) {
			natureTexte = STServiceLocator.getVocabularyService().getEntryLabel(
					VocabularyConstants.NATURE_TEXTE_DIRECTORY, texteMaitre.getNatureTexte());
		} else {
			natureTexte = "";
		}
	...
	private void computeProcedureVote(TexteMaitre texteMaitre) {
		if (texteMaitre.getProcedureVote() == null) {
			...
			if (PROCEDURE_VOTE_NORMALE.equals(texteMaitre.getProcedureVote())) {
				...
				procedureVote = STServiceLocator.getVocabularyService().getEntryLabel(
						VocabularyConstants.PROCEDURE_VOTE_DIRECTORY, texteMaitre.getProcedureVote());
			}
		}
	}
