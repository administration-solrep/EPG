...
public class OrdonnanceDTOImpl extends AbstractMapDTO implements OrdonnanceDTO {
	...
	@Override
	public DocumentModel remapField(TexteMaitre texteMaitre) throws ClientException {
	...
	@Override
	public void refresh(Dossier dossier, CoreSession session) throws ClientException {
		...
		ActiviteNormative activiteNormative = SolonEpgServiceLocator.getActiviteNormativeService()
				.findActiviteNormativeByNor(getNumeroNor(), session);

		if (activiteNormative == null || activiteNormative.getDocument() == null) {
			LOGGER.error(session, STLogEnumImpl.NPE_PARAM_METH_TEC, "le texte maitre avec nor " + getNumeroNor()
					+ " ne doit pas etre  null");
			throw new ClientException("le texte maitre avec nor " + getNumeroNor() + " ne doit pas etre  null");
		}
