...
public class TextesSignalesBatchListener extends AbstractBatchEventListener {
	...
	private static final String		QUERY	= "SELECT * FROM Dossier WHERE retdila:dateParutionJorf <= TIMESTAMP '%s' AND dos:dateVersementTS IS NOT NULL and ecm:isProxy = 0 ";
	...
	protected void processEvent(CoreSession session, Event event) throws ClientException {
		...
		try {
			STServiceLocator.getSuiviBatchService().createBatchResultFor(batchLoggerModel,
					"Suppression des textes signalés", nbTextesRetires, endTime - startTime);
		} catch (Exception e) {
