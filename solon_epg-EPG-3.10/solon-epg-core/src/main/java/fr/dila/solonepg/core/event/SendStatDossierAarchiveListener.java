...
public class SendStatDossierAarchiveListener implements PostCommitEventListener {
	...
	private void handleEvent(Event event) throws ClientException {
		...
		final List<DossierArchivageStatDTO> listDossiersArchives = (List<DossierArchivageStatDTO>) eventProperties
				.get(SolonEpgEventConstant.SEND_STAT_DOSSIER_ARCHIVE_LIST_DOS_PROPERTY);
		final String recipient = (String) eventProperties
				.get(SolonEpgEventConstant.SEND_STAT_DOSSIER_ARCHIVE_RECIPIENT_PROPERTY);
		final Date startDate = (Date) eventProperties
				.get(SolonEpgEventConstant.SEND_STAT_DOSSIER_ARCHIVE_STARTDATE_PROPERTY);
		final Date endDate = (Date) eventProperties
				.get(SolonEpgEventConstant.SEND_STAT_DOSSIER_ARCHIVE_ENDDATE_PROPERTY);
		...
		DataSource excelFile = ExcelUtil.exportStatDossiersArchives(listDossiersArchives);
		...
		final String content = "Bonjour, veuillez trouver en pièce jointe l'export demandé le "
				+ sdf.format(dateDemande) + " concernant la statistique sur les dossiers archivés au statut \"" + statutArchivage + "\" "
				+ "sur la période du " + sdf.format(startDate) + " au " + sdf.format(endDate) + ".";
