...
public class SuppressionNotificationListener extends AbstractBatchEventListener {
    private static final String QUERY = "SELECT nt.ecm:uuid as id FROM  NotificationDoc as nt WHERE nt.nt:dateArrive <= TIMESTAMP '%s' ";
    ...
    protected void processEvent(CoreSession session, Event event) throws ClientException {
        ...
        try {
        	STServiceLocator.getSuiviBatchService().createBatchResultFor(batchLoggerModel, "Suppression des notifications (maj corbeille)", nbSuppressionNotification, endTime-startTime);
        } catch (Exception e) {
