...
public class RequeteDossierSimpleImpl  extends STDomainObjectImpl implements RequeteDossierSimple{
    ...
    @Override
    public Integer getIdStatutArchivageDossier() {
        return  PropertyUtil.getIntegerProperty(this.document,REQUETE_DOSSIER_SIMPLE_CRITERES_PRINCIPAUX_SCHEMA, ID_STATUT_ARCHIVAGE_DOSSIER);
    }

    @Override
    public void setIdStatutArchivageDossier(Integer idStatutArchivageDossier) {
    ...
    public List<String> getFiletypeIds(){
        List<String> chosenFiletypeIds = new ArrayList<String>();
          for (EPGFulltextIndexEnum index: EPGFulltextIndexEnum.values()){
            if (index.isNeededBy(this)){
                chosenFiletypeIds.add(index.getFiletypeId().toString());
            }
        }
