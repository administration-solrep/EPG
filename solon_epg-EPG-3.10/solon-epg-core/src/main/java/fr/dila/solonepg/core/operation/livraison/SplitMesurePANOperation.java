 ...
 * 
 */
@Operation(id = SplitMesurePANOperation.ID, category = CaseConstants.CASE_MANAGEMENT_OPERATION_CATEGORY, label = "Mise à jour des mesures d'application des lois et d'habilitations du PAN", description = SplitMesurePANOperation.DESCRIPTION)
public class SplitMesurePANOperation {
	/**
	 * Identifiant technique de l'opération.
	 */
	public final static String		ID			= "SolonEpg.Livraison.SplitMesurePan";

	public final static String		DESCRIPTION	= "Cette opération initialise les mesures du pan pour séparer les mesures d'habilitations et les mesures d'application des lois";
	...
	public void run() throws ClientException {
		...
		final String queryLois = "select a.ecm:uuid as id from ActiviteNormative as a where a.norma:applicationLoi='1' OR a.norma:ordonnance38C = '1'";
		final List<DocumentModel> loisDocList = QueryUtils.doUFNXQLQueryAndFetchForDocuments(session,
				ActiviteNormativeConstants.ACTIVITE_NORMATIVE_DOCUMENT_TYPE, queryLois, null);
			...
			if (idsHabilitationLoi != null && !idsHabilitationLoi.isEmpty()) {
				...
				for (String idHabilitation : idsHabilitationLoi) {
					...
					if (idsMesureLoi != null && idsMesureLoi.contains(idHabilitation)) {
						...
						final DocumentModel copyHabilitation = session.copy(new IdRef(idHabilitation), new PathRef(
								ActiviteNormativeConstants.ACTIVITE_NORMATIVE_PATH), null);
