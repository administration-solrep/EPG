...
public class WsUtil {
	 ...
	 * @throws IOException
	 */
	public static DossierEpgWithFile getDossierEpgWithFileFromDossier(CoreSession session, Dossier dossier,
			boolean affichagePublicationEtEpreuvage) throws ClientException, IOException {
		...
		if (affichagePublicationEtEpreuvage) {
			...
			parapheurFilesDoc = parapheurService.getPieceParapheur(session, dossier, true);
			parapheurFilesDoc.addAll(parapheurService.getEpreuvesFiles(session, dossier));

		} else {
			parapheurFilesDoc = parapheurService.getParapheurFiles(session, dossier);
		}

		if (parapheurFilesDoc != null && parapheurFilesDoc.size() > 0) {
			ListeFichiers parapheurFichiers = new ListeFichiers();
			fillListeFichiersWsFromDocModelList(parapheurFichiers, parapheurFilesDoc,
		...
		List<DocumentModel> fddFilesDoc = new ArrayList<DocumentModel>();
		if (affichagePublicationEtEpreuvage) {
		...
		if (fddFilesDoc != null && fddFilesDoc.size() > 0) {
			...
			fillListeFichiersWsFromDocModelList(fondDossierFichiers, fddFilesDoc,
	 ...
	 * @throws IOException
	 */
	private static void fillListeFichiersWsFromDocModelList(ListeFichiers listeFichiersWs,
			List<DocumentModel> docModelList, String folderRoot) throws IOException {
	 ...
	 * @throws ClientException
	 */
	public static DossierEpg getDossierEpgFromDossier(Dossier dossier, CoreSession session,
			Boolean affichagePublicationEtEpreuvage) throws ClientException {
		...
		if (acteType != null) {
			...
			case ARRETE_MINISTERIEL:
				...
				fillArreteFromDossier(session, dossier, acteType, arreteMinisteriel, affichagePublicationEtEpreuvage);
			...
			case ARRETE_INTERMINISTERIEL:
				...
				fillArreteFromDossier(session, dossier, acteType, arreteInterministeriel,
			...
			case DECRET_PUBLICATION_TRAITE_OU_ACCORD:
				...
				fillDecretFromDossier(session, dossier, acteType, decretDePublicationDeTraiteOuAccord,
						affichagePublicationEtEpreuvage);
			...
			case INFORMATIONS_PARLEMENTAIRES:
				...
				// propriétés communes à tous les types d'actes
				fillActeFromDossier(session, (Acte) informationsParlementaires, acteType, dossier,
						affichagePublicationEtEpreuvage);
			...
			case LOI_ART_53_CONSTITUTION:
				...
				// propriétés communes à tous les types d'actes
				fillActeFromDossier(session, loiArt53Constitution, acteType, dossier, affichagePublicationEtEpreuvage);
			...
			case LOI_CONSTITUTIONNELLE:
				...
				loiConstitutionnelle.getTranspositionDirectiveEuropeenne().addAll(getTranspoDirEuropeenneList(dossier));
				// propriétés communes à tous les types d'actes
				fillActeFromDossier(session, loiConstitutionnelle, acteType, dossier, affichagePublicationEtEpreuvage);
			...
			case ARRETE_MINISTERIEL_INDIVIDUEL:
				...
				fillArreteFromDossier(session, dossier, acteType, arreteMinisterielInd,
						affichagePublicationEtEpreuvage);
			...
			case ARRETE_INTERMINISTERIEL_INDIVIDUEL:
				...
				fillArreteFromDossier(session, dossier, acteType, arreteInterministerielInd,
						affichagePublicationEtEpreuvage);
			...
			case DECRET_CE_INDIVIDUEL:
				...
				// propriétés communes à tous les autres types de décrets
				fillDecretFromDossier(session, dossier, acteType, decretCEIndividuel, affichagePublicationEtEpreuvage);
			...
			case DECRET_CE_CM_INDIVIDUEL:
				...
				// propriétés communes à tous les autres types de décrets
				fillDecretFromDossier(session, dossier, acteType, decretCECMIndividuel,
						affichagePublicationEtEpreuvage);
			...
			case DECRET_CM_INDIVIDUEL:
				...
				fillDecretFromDossier(session, dossier, acteType, decretCMIndividuel, affichagePublicationEtEpreuvage);
			...
			case DECRET_PR_INDIVIDUEL:
				...
				fillDecretFromDossier(session, dossier, acteType, decretPRIndividuel, affichagePublicationEtEpreuvage);
			...
			case DECRET_SIMPLE_INDIVIDUEL:
				...
				fillDecretFromDossier(session, dossier, acteType, decretSimpleIndividuel,
						affichagePublicationEtEpreuvage);
	 ...
	 * @throws ClientException
	 */
	public static BORDEREAU getBordereauFromDossier(Dossier dossier, CoreSession session,
			Boolean affichagePublicationEtEpreuvage) throws ClientException {
		...
		if(acteType != null) {
			switch (acteType) {
				...
				case ARRETE_MINISTERIEL_INDIVIDUEL:
					ArreteMinisterielIndividuel arreteMinisterielInd = new ArreteMinisterielIndividuel();
					fillArreteFromDossier(session, dossier, acteType, arreteMinisterielInd, affichagePublicationEtEpreuvage);
				...
				case DECRET_CE_CM_INDIVIDUEL:
					...
					decretCECMIndividuel.setSectionCe(getSectionCeFromDossier(dossier));
					// propriétés communes à tous les autres types de décrets
					fillDecretFromDossier(session, dossier, acteType, decretCECMIndividuel, affichagePublicationEtEpreuvage);
	...
	private static String getSectionPosteFrom(Dossier dossier, CoreSession session) {
		String section = null;
		List<String> sectionLabelListe = new ArrayList<String>();
		try {
			DocumentModel dossierDocModel = dossier.getDocument();
			// récupération de la feuille de route
			DocumentModel feuilleRouteDocModel = SolonEpgServiceLocator.getDossierDistributionService()
					.getLastDocumentRouteForDossier(session, dossierDocModel);
			// récupération des étapes de feuille de route courante
			List<DocumentModel> runningStep = SolonEpgServiceLocator.getFeuilleRouteService().getRunningSteps(session,
					feuilleRouteDocModel.getId());
			if (runningStep != null && runningStep.size() > 0) {
				...
				final EpgOrganigrammeService epgOrganigrammeService = SolonEpgServiceLocator
				...
				for (DocumentModel documentModel : runningStep) {
					...
					STRouteStep routeStep = documentModel.getAdapter(STRouteStep.class);
					...
					if (isPosteCe) {
						...
						if (sectionNodeList != null && sectionNodeList.size() == 1) {
							...
							for (OrganigrammeNode uniteStructurelleNode : sectionNodeList) {
								List<EntiteNode> ministereNodeListFromParentNode = ministeresService
										.getMinistereParentFromUniteStructurelle(uniteStructurelleNode.getId());
	...
	private static SectionCe getSectionCeFromDossier(Dossier dossier) {
		...
		sectionCe.setDateTransmissionSectionCe(DateUtil.calendarToXMLGregorianCalendar(conseilEtat
				.getDateTransmissionSectionCe()));
	 ...
	 * @throws ClientException
	 */
	private static void fillDecretFromDossier(CoreSession session, Dossier dossier, ActeType acteType, Decret decret,
			Boolean affichagePublicationEtEpreuvage) throws ClientException {
	 ...
	 * @throws ClientException
	 */
	private static void fillArreteFromDossier(CoreSession session, Dossier dossier, ActeType acteType, Arrete arrete,
			Boolean affichagePublicationEtEpreuvage) throws ClientException {
	...
	private static List<TranspositionDirectiveEuropeenne> getTranspoDirEuropeenneList(Dossier dossier) {
		List<ComplexeType> transpositionDirectiveList = dossier.getTranspositionDirective();
		List<TranspositionDirectiveEuropeenne> transpoDirEuropeenneList = new ArrayList<TranspositionDirectiveEuropeenne>();
		...
		for (ComplexeType transpositionDirective : transpositionDirectiveList) {
			...
			transpositionDirectiveEuropeenne.setCommentaire((String) transpositionDirectiveMap
					.get(DossierSolonEpgConstants.DOSSIER_COMPLEXE_TYPE_COMMENTAIRE_PROPERTY));
			transpositionDirectiveEuropeenne.setReferenceDirective((String) transpositionDirectiveMap
					.get(DossierSolonEpgConstants.DOSSIER_COMPLEXE_TYPE_REF_PROPERTY));
			transpositionDirectiveEuropeenne.setTitreDirective((String) transpositionDirectiveMap
	...
	private static List<MesureApplicationLoi> getMesureApplicationLoiList(Dossier dossier) {
		List<ComplexeType> applicationsLois = dossier.getApplicationLoi();
		List<MesureApplicationLoi> mesureApplicationLoiList = new ArrayList<MesureApplicationLoi>();
		...
		for (ComplexeType applicationLoi : applicationsLois) {
			...
			// récupération des propriétés d'une transposition de directive européenne
			mesureApplicationLoi.setCommentaire((String) mesureApplicationLoiMap
					.get(DossierSolonEpgConstants.DOSSIER_COMPLEXE_TYPE_COMMENTAIRE_PROPERTY));
			mesureApplicationLoi.setReferenceLoi((String) mesureApplicationLoiMap
					.get(DossierSolonEpgConstants.DOSSIER_COMPLEXE_TYPE_REF_PROPERTY));
			mesureApplicationLoi.setReferenceMesure((String) mesureApplicationLoiMap
					.get(DossierSolonEpgConstants.DOSSIER_LOI_APPLIQUEE_REF_MESURE_PROPERTY));
			mesureApplicationLoi.setTitreLoi((String) mesureApplicationLoiMap
					.get(DossierSolonEpgConstants.DOSSIER_COMPLEXE_TYPE_TITRE_PROPERTY));
			// ajout à la liste
	...
	private static List<TranspositionOrdonnance> getTranspositionOrdonnanceList(Dossier dossier) {
		List<ComplexeType> transpositionsOrdonnaces = dossier.getTranspositionOrdonnance();
		List<TranspositionOrdonnance> transpositionOrdonnanceList = new ArrayList<TranspositionOrdonnance>();
		...
		for (ComplexeType transOrdonnance : transpositionsOrdonnaces) {
			...
			// récupération des propriétés d'une transposition de directive européenne
			transpositionOrdonnance.setCommentaire((String) transpositionOrdonnanceMap
					.get(DossierSolonEpgConstants.DOSSIER_COMPLEXE_TYPE_COMMENTAIRE_PROPERTY));
			transpositionOrdonnance.setReferenceOrdonnance((String) transpositionOrdonnanceMap
					.get(DossierSolonEpgConstants.DOSSIER_COMPLEXE_TYPE_REF_PROPERTY));
			transpositionOrdonnance.setTitreOrdonnance((String) transpositionOrdonnanceMap
					.get(DossierSolonEpgConstants.DOSSIER_COMPLEXE_TYPE_TITRE_PROPERTY));
			// ajout à la liste
	...
	/**
	 * Récupération du type de publication : in extenso ou version intégrale
	 * 
	 * @param dossier
	 *            dossier
	 ...
	 * @throws ClientException
	 */
	public static void fillActeFromDossier(CoreSession session, Acte acte, ActeType acteType, Dossier dossier,
			Boolean affichagePublicationEtEpreuvage) throws ClientException {
		...
		if (!dossier.getVecteurPublication().isEmpty()) {
			List<String> vecteurs = new ArrayList<String>();
	...
	public static void fillStatutActeFromDossier(Acte acte, Dossier dossier) {
		...
		if (statutArchivage != null) {
			if (!VocabularyConstants.STATUT_ARCHIVAGE_AUCUN.equals(statutArchivage)
	 ...
	 * @throws ClientException
	 */
	public static void createFilesInFdd(CoreSession session, DocumentModel dossierDoc,
			FondDeDossierService fondDeDossierService, List<Fichier> fichierList, String folderName)
			throws ClientException {
		...
		for (Fichier fichier : fichierList) {
			fondDeDossierService.createFondDeDossierFile(session, fichier.getNom(),
					new ByteArrayBlob(fichier.getContenu()), folderName, dossierDoc);
		}
	}
	 ...
	 * @throws ClientException
	 */
	public static void createFileInFdd(CoreSession session, DocumentModel dossierDoc, PEFichier fichier,
		...
		List<DocumentModel> lstDocsEpreuves = fondDeDossierService.getEpreuvesFiles(session,
				dossierDoc.getAdapter(Dossier.class));
		String filename = fichier.getNom().replaceAll("'", "_");

		// Si on n'a pas de fichiers d'épreuvage, on le crée dans le fond de dossier
		if (lstDocsEpreuves == null || lstDocsEpreuves.isEmpty()) {
			fondDeDossierService.createFondDeDossierFile(session, filename, fichier.getContent(), folderName,
					dossierDoc);
		} else {
			// Sinon on met à jour sa version dans le fond de dossier
			// On ne met à jour que le premier document car le fond de dossier peut avoir contenu plusieurs documents

			final ByteArrayBlob blobArray = new ByteArrayBlob(fichier.getContent());
			fondDeDossierService.updateFile(session, lstDocsEpreuves.get(0), filename, blobArray,
					(NuxeoPrincipal) session.getPrincipal(), dossierDoc);
		}
	 ...
	 * @throws ClientException
	 */
	public static void createOrUpdateFileListInFdd(CoreSession session, DocumentModel dossierDoc, 
			List<Fichier> fichierList, String folderName) throws ClientException {
		FondDeDossierService fondDeDossierService = SolonEpgServiceLocator.getFondDeDossierService();
		List<DocumentModel> fddDocList = fondDeDossierService.getAvisCEFiles(session, dossierDoc.getAdapter(Dossier.class));
		Map<Fichier, DocumentModel> fichierUpdateMap = new HashMap<Fichier, DocumentModel>();
		List<Fichier> fichierCreate = new ArrayList<Fichier>();
		...
		if (fddDocList == null || fddDocList.isEmpty()) {
			...
			for (Map.Entry<Fichier, DocumentModel> entry : fichierUpdateMap.entrySet()) {
				fondDeDossierService.updateFile(session, entry.getValue(), entry.getKey().getNom(), 
						new ByteArrayBlob(entry.getKey().getContenu()), 
						(NuxeoPrincipal) session.getPrincipal(), dossierDoc);
			}
		}
	}
	...
	/**
	 * Récupère un fichier et enregistre ce fichier dans le répertoire du parapheur ayant le nom choisi.
	 * 
	 * @param dossierDoc
	 ...
	 * @throws ClientException
	 */
	public static void createFileInParapheur(CoreSession session, DocumentModel dossierDoc, PEFichier fichier,
			String folderName) throws ClientException {

		ParapheurService parapheurService = SolonEpgServiceLocator.getParapheurService();
		List<DocumentModel> lstDocsEpreuves = parapheurService.getEpreuvesFiles(session,
				dossierDoc.getAdapter(Dossier.class));
		String filename = fichier.getNom().replaceAll("'", "_");

		// Si on n'a pas encore de fichier dans le répertoire on le crée
		if (lstDocsEpreuves == null || lstDocsEpreuves.isEmpty()) {
			parapheurService.createParapheurFile(session, filename, fichier.getContent(), folderName, dossierDoc);
		} else {

			// Sinon on met à jour sa version dans le fond de dossier
			// On n'est sensé n'avoir qu'un seul document dans le parapheur mais au cas où on précise

			final ByteArrayBlob blobArray = new ByteArrayBlob(fichier.getContent());

			parapheurService.updateFile(session, lstDocsEpreuves.get(0), filename, blobArray,
					(NuxeoPrincipal) session.getPrincipal(), dossierDoc);
		}
	...
	public static void remapDossier(CoreSession session, Dossier dossier, DossierEpg dossierEpg, ActeType acteType)
		...
		if (ActeType.AVIS.equals(acteType)) {
			...
			if (avisPub != null) {
				if (PublicationIntOuExtType.IN_EXTENSO.equals(avisPub)) {
					dossier.setPublicationIntegraleOuExtrait(VocabularyConstants.TYPE_PUBLICATION_IN_EXTENSO);
		...
		if (acteIndex != null) {
			...
			if (acteIndexMotsCles != null && !acteIndexMotsCles.isEmpty()) {
				List<String> motClesList = indexationService.findAllIndexationMotCleForDossier(session,
		...
		if (acteVectPub != null && !acteVectPub.isEmpty()) {
			List<String> lstVectEtBulletinConvertis = new ArrayList<String>();
			...
			for (String element : acteVectPub) {
				...
				if (StringUtil.isNotBlank(idVecteur)) {
		...
		if (acteParuBO != null && !acteParuBO.isEmpty()) {
			List<fr.dila.solonepg.api.cases.typescomplexe.ParutionBo> parutionsBoDossier = new ArrayList<fr.dila.solonepg.api.cases.typescomplexe.ParutionBo>();
