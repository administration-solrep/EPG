    ...
    public void createContentStructure(DocumentModel eventDoc) throws ClientException {
        ...
        for (TemplateItemDescriptor item : template) {
            ...
            if (item.getTypeName().equals(SolonEpgFondDeDossierConstants.FOND_DE_DOSSIER_MODEL_ROOT_DOCUMENT_TYPE)){
                ...
                fondDeDossierModelRoot.setFormatsAutorises(DossierSolonEpgConstants.FormatAutoriseListExtendFolderFondDeDossier);
            } else if (item.getTypeName().equals(SolonEpgParametrageApplicationConstants.PARAMETRAGE_APPLICATION_DOCUMENT_TYPE)){
                ParametrageApplication parametrageApplication = newChild.getAdapter(ParametrageApplication.class);
                parametrageApplication.setMetadonneDisponibleColonneCorbeille(SolonEpgParametrageApplicationConstants.PARAMETRAGE_APPLICATION_DEFAULT_ESPACE_TRAITEMENT_COLUMN_LIST);
            }
