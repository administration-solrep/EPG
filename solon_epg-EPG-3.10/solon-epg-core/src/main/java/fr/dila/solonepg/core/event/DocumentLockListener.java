...
public class DocumentLockListener extends AbstractFilterEventListener<InlineEventContext> {
    ...
    @Override
    protected void doHandleEvent(Event event, final InlineEventContext context) throws ClientException {
    ...
    private void lockDocument(CoreSession session, NuxeoPrincipal principal, DocumentModel oldDoc, DocumentModel newDoc) {
    	...
    	try {
	        ...
	        if (STConstant.FEUILLE_ROUTE_DOCUMENT_TYPE.equals(docType)) {
	            ...
	            if (feuilleRoute.isModel() && principal.isMemberOf(STBaseFunctionConstant.FEUILLE_DE_ROUTE_MODEL_UDPATER)) {
