...
public class FullTextUtil extends fr.dila.st.core.util.FullTextUtil {
    ...
    public static Pattern getHasNumeroNorField() {
        if (hasNumeroNorField == null) {
            hasNumeroNorField = Pattern.compile(".*" + DossierSolonEpgConstants.DOSSIER_NUMERO_NOR_QUERY_LIKE + "{1}.*", Pattern.CANON_EQ);
        }
    ...
    public static Pattern getHasNumeroNorFieldWithNoFullText() {
        if (hasNumeroNorFieldWithNoFullText == null) {
            hasNumeroNorFieldWithNoFullText = Pattern.compile(".*([ \\(]|[d][\\.])" + DossierSolonEpgConstants.DOSSIER_NUMERO_NOR_QUERY_LIKE + "{1}.*", Pattern.CANON_EQ);
        }
    ...
    public static Pattern getBeginWithStarFullTextField() {
        if (beginWithStarFullTextField == null) {
            // TODO rendre la regex générique à tout les champs fullTexte
            beginWithStarFullTextField = Pattern.compile(".*" + DossierSolonEpgConstants.DOSSIER_NUMERO_NOR_QUERY_LIKE + "+[ ]*['\"]\\*{1}.*", Pattern.CANON_EQ);
        }
    ...
    public static String defineNumeroNorFieldAsFullTextField(String query) {
        if (hasNumeroNorFieldWithNoFullText(query)) {
            query = StringUtils.replace(query, " " + DossierSolonEpgConstants.DOSSIER_NUMERO_NOR_QUERY_LIKE, " " + DossierSolonEpgConstants.DOSSIER_NUMERO_NOR_PREFIX_FULL_TEXT);
            query = StringUtils.replace(query, "(" + DossierSolonEpgConstants.DOSSIER_NUMERO_NOR_QUERY_LIKE, "(" + DossierSolonEpgConstants.DOSSIER_NUMERO_NOR_PREFIX_FULL_TEXT);
            query = StringUtils.replace(query, "d." + DossierSolonEpgConstants.DOSSIER_NUMERO_NOR_QUERY_LIKE, "d." + DossierSolonEpgConstants.DOSSIER_NUMERO_NOR_PREFIX_FULL_TEXT);
        }
        return query;
    }
