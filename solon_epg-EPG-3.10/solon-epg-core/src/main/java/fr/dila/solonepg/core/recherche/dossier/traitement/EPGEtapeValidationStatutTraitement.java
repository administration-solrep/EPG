...
public class EPGEtapeValidationStatutTraitement implements RequeteTraitement<RequeteDossierSimple>{
    ...
    public void doBeforeQuery(RequeteDossierSimple requeteSimple) {
        ...
        if (statuts.length != 2){
            LOGGER.error("Le traitement EPGEtapeValidationStatutTraitement a échoué, vérifier la forme des identifiants dans le fichier statut_etape_recherche_statut.csv");
