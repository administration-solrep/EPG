	...
	protected void processEvent(CoreSession session, Event event) throws ClientException {
		...
		try {
			...
			final String query = "SELECT * FROM Dossier WHERE ecm:currentLifeCycleState = 'running' and dos:"
					...
					+ VocabularyConstants.CANDIDAT_ABANDON + "' and dos:"
					+ DossierSolonEpgConstants.DOSSIER_STATUT_PROPERTY + " = '" + VocabularyConstants.STATUT_LANCE
					+ "' and  dos:" + DossierSolonEpgConstants.DOSSIER_DATE_CANDIDATURE_PROPERTY + " < TIMESTAMP '"
					+ literalDate + " ' and ecm:isProxy = 0";
		...
		try {
			STServiceLocator.getSuiviBatchService().createBatchResultFor(batchLoggerModel, "Abandon de dossiers",
					nbDossierAbandon, endTime - startTime);
		} catch (Exception e) {
