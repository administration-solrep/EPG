...
public class InformationsParlementairesServiceImpl implements InformationsParlementairesService {
	...
	public void callWsTransmissionDatePublicationJOEpp(CoreSession session, Dossier dossier) throws Exception {
		...
		if (webservice != null) {
			...
			XMLGregorianCalendar datePublication = DateUtil.calendarToXMLGregorianCalendar(retourDila
					.getDateParutionJorf());
			...
			if (TraitementStatut.OK.equals(response.getStatut())) {
				...
				// Statut KO
				LOGGER.error(session, EpgLogEnumImpl.FAIL_GET_WS_EPP_TEC, "Appel ws transmissionDatePublication epp KO"
						+ response.getMessageErreur());
				throw new ClientException("Retour KO du ws TransmissionDatePublicationJO");
			}
		} else {
			LOGGER.error(session, EpgLogEnumImpl.FAIL_GET_WS_EPP_TEC,
					"Impossible de se connecter à EPP pour appeler TransmissionDatePublicationJO");
			throw new ClientException("Impossible de se connecter à EPP pour appeler TransmissionDatePublicationJO");
		}
	}
	...
	public void callWsAccuserReception(CoreSession session, Dossier dossier, Object wsEvenement) throws Exception {
		...
		if (webservice != null) {
			...
			if (TraitementStatut.OK.equals(response.getStatut())) {
				...
				// Statut KO
				LOGGER.info(EpgLogEnumImpl.FAIL_GET_WS_EPP_TEC,
						"Appel ws AccuserReception epp KO : " + response.getMessageErreur());
				throw new ClientException("Retour KO du ws AccuserReception");
			}
		} else {
			LOGGER.info(EpgLogEnumImpl.FAIL_GET_WS_EPP_TEC,
					"Impossible de se connecter à EPP pour appeler AccuserReception");
			throw new ClientException("Impossible de se connecter à EPP pour appeler AccuserReception");
		}
	}
	...
	public void callWsCreerVersionEvt48(CoreSession session, Dossier dossier) throws Exception {
		...
		if (webservice != null) {
			...
			evt.setDestinataire("Sénat".equals(dossier.getEmetteur()) ? Institution.SENAT : Institution.ASSEMBLEE_NATIONALE);
			...
			Integer pageJo = retourDila.getPageParutionJorf() != null ? retourDila.getPageParutionJorf().intValue() : null;
			...
			if (TraitementStatut.OK.equals(response.getStatut())) {
				...
				LOGGER.info(EpgLogEnumImpl.FAIL_GET_WS_EPP_TEC,
						"Appel ws CreerVersion epp KO : " + response.getMessageErreur());
				throw new ClientException("Retour KO du ws CreerVersion");
			}
		} else {
			LOGGER.info(EpgLogEnumImpl.FAIL_GET_WS_EPP_TEC,
					"Impossible de se connecter à EPP pour appeler CreerVersion");
			throw new ClientException("Impossible de se connecter à EPP pour appeler CreerVersion");
		}
	}

	private WSEpp getWsEpp(CoreSession session) throws WSProxyFactoryException, ClientException {
		String url = null;
		...
		ParametrageApplication parametrageApplication = parametreApplicationService
		...
		if (StringUtils.isNotBlank(url) && url.contains("/site/solonepp")) {
			...
			password = parametrageApplication.getPassEppInfosParl();
		} else {
			url = null;
		}
		...
		return factory.getService(WSEpp.class);
	}

	public WSEvenement getWsEvenement(CoreSession session) throws WSProxyFactoryException, ClientException {
		String url = null;
		String username = null;
		String password = null;
		String keyAlias = null;
		...
		WSProxyFactory factory = new WSProxyFactory(url, null, username, password, keyAlias);
		return factory.getService(WSEvenement.class);
	}
