...
public class FeuilleRouteSubstitutionListener extends RollbackEventListener {
    ...
    public void handleInlineEvent(Event event, InlineEventContext ctx) throws ClientException {
        ...
        final DocumentModel oldFeuilleRouteDoc = (DocumentModel) ctx.getProperty(STEventConstant.DOSSIER_DISTRIBUTION_OLD_ROUTE_EVENT_PARAM);
        final DocumentModel newFeuilleRouteDoc = (DocumentModel) ctx.getProperty(STEventConstant.DOSSIER_DISTRIBUTION_NEW_ROUTE_EVENT_PARAM);
        String typeCreation = (String) ctx.getProperty(STEventConstant.DOSSIER_DISTRIBUTION_SUBSTITUTION_ROUTE_TYPE_EVENT_PARAM);
        ...
        dossierDistributionService.startRouteAfterSubstitution(session, oldFeuilleRouteDoc, newFeuilleRouteDoc, typeCreation);
     }
}
