...
public class InitCaseRootBatchListener extends AbstractBatchEventListener {
	...
	protected void processEvent(CoreSession session, Event event) throws ClientException {
		...
		for (int i = 0; i <= NEXT_DAYS_CHECK; i++) {
			...
			CaseTreeHelper.getOrCreateDateTreeFolder(session, mailRootdoc, calendarToCheck.getTime(),
		...
		suiviBatchService.createBatchResultFor(batchLoggerModel, "Mise à jour du plan de classement", endTime
				- startTime);
