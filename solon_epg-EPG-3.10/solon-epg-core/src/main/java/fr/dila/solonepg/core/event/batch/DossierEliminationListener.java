...
public class DossierEliminationListener extends AbstractBatchEventListener {
	...
	protected void processEvent(CoreSession session, Event event) throws ClientException {
		...
		try {
			...
			// STParametreConstant.DELAI_ELIMINATION_DOSSIERS).getUnit();
			...
			final String query = "SELECT * FROM Dossier WHERE ecm:currentLifeCycleState = 'init' and dos:"
					...
					+ VocabularyConstants.STATUT_INITIE + "' and  dc:modified < TIMESTAMP '" + literalDate
					+ " ' and ecm:isProxy = 0 ";
		...
		try {
			STServiceLocator.getSuiviBatchService().createBatchResultFor(batchLoggerModel,
					"Dossiers candidats élimination", nbEliminationCandidat, endTime - startTime);
		} catch (Exception e) {
