...
public class DeleteExportPanStatsBatchListener extends AbstractBatchEventListener {
    ...
    private static final String QUERY = "Select e.ecm:uuid as id from ExportPANStat as e WHERE e.expanstat:dateRequest <= TIMESTAMP '%s' ";
    ...
    protected void processEvent(CoreSession session, Event event) throws ClientException {
        ...
        try {
	        ...
	        List<DocumentModel> exportsTargets = QueryUtils.doUFNXQLQueryAndFetchForDocuments(session, ActiviteNormativeStatsConstants.EXPORT_PAN_STAT_TYPE, query, new Object[] {});
	        ...
	        if (exportsTargets != null && !exportsTargets.isEmpty()) {
				...
				for (DocumentModel exportDoc : exportsTargets) {
					...
					String legislatures = PropertyUtil.getStringProperty(exportDoc, ActiviteNormativeStatsConstants.EXPORT_PAN_STAT_SCHEMA,
					...
					String folderPath = ActiviteNormativeReportsGenerator.getFilePath(currentLegislature, export.getOwner(), export.getName());
        ...
        try {
        	STServiceLocator.getSuiviBatchService().createBatchResultFor(batchLoggerModel, "Suppression des Statistiques PAN exportées", nbSuppression, endTime-startTime);
        } catch (Exception e) {
