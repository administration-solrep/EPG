...
public class TableauDynamiqueServiceImpl implements TableauDynamiqueService {
	...
	@Override
	public List<DocumentModel> findAll(final CoreSession session) throws ClientException {
		...
		return QueryUtils.doUnrestrictedUFNXQLQueryAndFetchForDocuments(session, "TableauDynamique",
				queryBuilder.toString(), params);

	}

	@Override
	public List<DocumentModel> findMine(final CoreSession session) throws ClientException {
	...
	@Override
	public Map<Integer, String> createTableauDynamique(final CoreSession session, final String type,
			final String query, final List<String> idDestinataires, final String intitule) throws ClientException {
		List<String> splitedDestinataireIds = new ArrayList<String>();
		for (String destinataireId : idDestinataires) {
		...
		if (!users.get(0).isEmpty()) {
			final DocumentModel tabModel = new DocumentModelImpl(
					SolonEpgEspaceRechercheConstants.TABLEAU_DYNAMIQUE_PATH, UUID.randomUUID().toString(),
			...
			tableauDynamique.setDestinatairesId(idDestinataires);
		...
		result.put(0, StringUtil.join(users.get(0).toArray(new String[users.get(0).size()]), ", ", ""));
		result.put(1, StringUtil.join(users.get(1).toArray(new String[users.get(1).size()]), ", ", ""));
		return result;
	}

	@Override
	public Map<Integer, String> updateTableauDynamique(final CoreSession session, final DocumentModel tabModel,
			final List<String> idDestinataires, final String intitule) throws ClientException {
		final Map<Integer, String> result = new HashMap<Integer, String>();
	...
	private Map<Integer, Set<String>> getUsersName(final CoreSession session, final List<String> idDestinataires,
			final String tabModelId) throws ClientException {
				...
				if (e != null) {
					...
					if (e != null) {
						...
						if (e != null) {
							...
							throw new ClientException("Impossible de retrouve le type de noeud pour " + idDestinataire);
						}
					}
				}
			}
		...
		final ParametrageApplication parametrageApplication = parametreApplicationService
		...
		if (parametrageApplication != null) {
			for (final String userName : usersName) {
				...
				if (getCountForUser(session, userName, tabModelId).intValue() < parametrageApplication
						.getNbTableauxDynamiques().intValue()) {
	...
	private Long getCountForUser(final CoreSession session, final String userName, final String tabModelId)
		...
		if (tabModelId == null) {
			final Object params[] = new Object[] { userName, userName };
			return QueryUtils.doCountQuery(session, FlexibleQueryMaker.KeyCode.UFXNQL.key + queryBuilder.toString(),
					params);
		} else {
	...
	@Override
	public Boolean userIsUnderQuota(final CoreSession session, final String userName) throws ClientException {
		final ParametreApplicationService parametreApplicationService = SolonEpgServiceLocator
				.getParametreApplicationService();
		final ParametrageApplication parametrageApplication = parametreApplicationService
				.getParametreApplicationDocument(session);

		if (getCountForUser(session, userName, null).intValue() < parametrageApplication.getNbTableauxDynamiques()
				.intValue()) {
			return true;
		} else
			return false;
	...
	@Override
	public void deleteTableauDynamique(final CoreSession session, final DocumentModel tabModel) throws ClientException {
