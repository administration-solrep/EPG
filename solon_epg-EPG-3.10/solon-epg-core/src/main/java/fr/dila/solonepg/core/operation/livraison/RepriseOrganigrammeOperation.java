...
import fr.dila.solonepg.api.constant.SolonEpgOrganigrammeConstant;
...
public class RepriseOrganigrammeOperation extends RepriseLDAP {
	public static final String	ID	= "SolonEpg.Organigramme.Reprise";
	...
	protected List<EntiteNode> getAllEntites() {
		...
		try {
			...
			for (DocumentModel group : entries) {
				...
				DocumentModel loadedGroup = session.getEntry((String) group.getProperty(session.getDirectory()
						.getSchema(), STConstant.LDAP_GROUP_NAME_PROPERTY));
				EntiteNodeImpl node = new EntiteNodeImpl();
				...
				node.setMembreGouvernement(PropertyUtil.getStringProperty(loadedGroup,
						STSchemaConstant.ORGANIGRAMME_ENTITE_SCHEMA, "membreGouvernement"));

				node.setNorMinistere(PropertyUtil.getStringProperty(loadedGroup,
						STSchemaConstant.ORGANIGRAMME_ENTITE_SCHEMA, SolonEpgOrganigrammeConstant.NOR_MINISTERE));

				String suiviActiviteNormative = PropertyUtil.getStringProperty(loadedGroup,
						STSchemaConstant.ORGANIGRAMME_ENTITE_SCHEMA,
						SolonEpgOrganigrammeConstant.SUIVI_ACTIVITE_NORMATIVE);
				node.setSuiviActiviteNormative(Boolean.parseBoolean(suiviActiviteNormative));
	...
	protected Map<String, UniteStructurelleNode> getAllUnitesAndDirections() {
		...
		try {
			...
			for (DocumentModel group : entries) {
				DocumentModel loadedGroup = session.getEntry((String) group.getProperty(session.getDirectory()
						.getSchema(), STConstant.LDAP_GROUP_NAME_PROPERTY));
				UniteStructurelleNodeImpl node = new UniteStructurelleNodeImpl();
				...
				List<String> norRefStringList = PropertyUtil.getStringListProperty(loadedGroup,
						STSchemaConstant.ORGANIGRAMME_UNITE_STRUCTURELLE_SCHEMA,
						SolonEpgOrganigrammeConstant.NOR_DIRECTION);
				for (String norRefString : norRefStringList) {
					norRefList.add(new NorDirection(norRefString));
				}
				node.setNorDirectionList(norRefList);
	...
	protected List<PosteNode> getAllPostes() {
		...
		try {
			...
			for (DocumentModel group : entries) {
				...
				node.setSuperviseurSGG(false);

				node.setPosteWs(false);
