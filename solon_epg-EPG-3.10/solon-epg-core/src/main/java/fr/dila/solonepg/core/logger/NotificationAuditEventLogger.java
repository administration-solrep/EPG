...
public class NotificationAuditEventLogger extends STNotificationAuditEventLogger {
	 ...
	 */
	protected static final Set<String>	SET_EVENT_PAN		= new HashSet<String>();
	...
	static {
		...
		SET_EVENT_ADMIN.add(SolonEpgEventConstant.CREATE_MODELE_FDR_EVENT);
		SET_EVENT_ADMIN.add(SolonEpgEventConstant.UPDATE_MODELE_FDR_EVENT);
		SET_EVENT_ADMIN.add(SolonEpgEventConstant.DELETE_MODELE_FDR_EVENT);
		SET_EVENT_ADMIN.add(SolonEpgEventConstant.DUPLICATION_MODELE_FDR_EVENT);
	...
	protected void loggerProcess(Event event) throws ClientException {
		...
		if (event.getContext() instanceof DocumentEventContext) {
			...
			if (model != null) {
				...
				} else if (EventNames.afterCaseSentEvent.name().equals(eventName)
						&& DossierSolonEpgConstants.DOSSIER_DOCUMENT_TYPE.equals(docType)) {
					// affectation du dossier à un ministere
					docCtx.setComment("Transfert du dossier à ["
	 ...
	 */
	private void logFeuilleDeRoute(Event event, DocumentEventContext context) throws ClientException {
	...
	private void logDossier(Event event, DocumentEventContext context) {
		...
		Object[] datas = new Object[] { event.getName(), context.getPrincipal().getName(), dossier.getMinistereResp(),
				dossier.getNomRespDossier() + " " + dossier.getPrenomRespDossier(), dossier.getNumeroNor() };
	...
	protected Object[] getFileData(Event event, DocumentEventContext context) {
		...
		return new Object[] { event.getName(), context.getPrincipal().getName(), context.getComment(),
				model.getParentRef(), "" };
