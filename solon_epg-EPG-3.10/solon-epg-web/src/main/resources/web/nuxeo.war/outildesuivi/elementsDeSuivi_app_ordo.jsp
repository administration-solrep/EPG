...
$(document).ready(function() {
   //$("#tauxAppDiv").load("openDocument.jsp?fileName=tauxDapplicationAuFilDeLeauParOrdonnanceAffichageAvencee-<%//=loiNb%>");
   $.post("openDocument.jsp", {fileName:"tauxDapplicationAuFilDeLeauParOrdonnanceAffichageAvencee-<%=loiNb%>",typeLegislature:"<%=legisType%>"}, function(data){$("#tauxAppDiv").html(data);});
		...
		<tr height="60px">
			<td  class="mainTd">
				...
				<span class="isLink" onclick="reportsDivLoad('tableauDeSuivi-<%=loiNb%>');">
					&gt;&nbsp;Consulter le tableau de suivi des mesures d'application de l'ordonnance
				</span>
				<br/>
				<span class="isLink" onclick="reportsDivLoad('mesuresApplicaticationParOrdoAppOrdoNonPubliees-<%=loiNb%>');">
					&gt;&nbsp;Consulter le tableau de suivi des seules mesures restant &#224; prendre
				</span>
				<br/>
				<span class="isLink" onclick="reportsDivLoad('delai_ordo_tous_app_ordo');">
