...
@Operation(id = AddDecretToMesure.ID, category = Constants.CAT_DOCUMENT, label = "Attach Decret To Loi", description = "Attach Decret To Loi")
public class AddDecretToMesure {
    ...
    public void run() throws Exception {
        ...
        SolonEpgServiceLocator.getActiviteNormativeService().addDecretToMesureReprise(texteMaitreLoiId, mesureId, decretDTO, session);
        repriseLog.debug("Add Decert To Mesure is OK - numeroNor"+numeroNor);
        }catch (Exception e) {
            repriseLog.error("Add Decert To Mesure is KO - numeroNor= "+numeroNor,e);
            throw e ;
        }
    }
}
