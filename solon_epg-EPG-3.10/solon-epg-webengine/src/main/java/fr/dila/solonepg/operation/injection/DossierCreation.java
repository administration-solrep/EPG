...
@Operation(id = DossierCreation.ID, category = Constants.CAT_DOCUMENT, label = "Create Dossier", description = "Create Dossier : Test.")
public class DossierCreation {
	...
	public DocumentModel run() throws Exception {
		...
		try {
			...
			Case emptyCase = STServiceLocator.getCaseDistributionService().createEmptyCase(session, dossierDoc,
					currentMailbox);
			...
			try {
				...
				if (sectionCe != null && !sectionCe.trim().equals("")) {
					OrganigrammeNode uniteStructurelleNode = STServiceLocator.getSTUsAndDirectionService()
				...
				repriseLog.warn("la date est mal parsee: " + System.getProperty("line.separator")
						+ "dateTransmissionSectionCe = " + dateTransmissionSectionCe
						+ System.getProperty("line.separator") + "dateSectionCe = " + dateSectionCe
						+ System.getProperty("line.separator") + "dateAgCe = " + dateAgCe
						+ System.getProperty("line.separator") + " dateSignature = " + dateSignature
						+ System.getProperty("line.separator") + " datePreciseePublication = "
						+ datePreciseePublication + System.getProperty("line.separator") + " publicationDate = "
						+ publicationDate + System.getProperty("line.separator") + " dateSignaturePm = "
						+ dateSignaturePm + System.getProperty("line.separator") + " dateSignaturePr = "
						+ dateSignaturePr);
			}

			List<String> indexationRubriqueList = new ArrayList<String>();
			...
			if (nomCompletChargesMissions != null && !nomCompletChargesMissions.trim().equals("")) {
				List<String> nomCompletChargesMissionsList = new ArrayList<String>();
			...
			if (nomCompletConseillersPms != null && !nomCompletConseillersPms.trim().equals("")) {
				List<String> nomCompletConseillersPmsList = new ArrayList<String>();
			...
			if (StringUtils.isNotBlank(loiDeRef)) {
				...
				for (String loiDeRef : loiDeRefList) {
					List<ComplexeType> listApplicationLoi = new ArrayList<ComplexeType>();
					ComplexeType applicationLoi = new ComplexeTypeImpl();
					Map<String, Serializable> applicationLoiMap = new HashMap<String, Serializable>();
					applicationLoiMap.put("ref", loiDeRef);
					applicationLoi.setSerializableMap(applicationLoiMap);
					listApplicationLoi.add(applicationLoi);
					dossier.setApplicationLoi(listApplicationLoi);
				}
			...
			if (StringUtils.isNotBlank(ordoDeRef)) {
				...
				for (String transpositionOrdonnanceRef : transpositionOrdonnanceListRef) {
					List<ComplexeType> listTranspositionOrdonnance = new ArrayList<ComplexeType>();
					ComplexeType transpositionOrdonnance = new ComplexeTypeImpl();
					Map<String, Serializable> transpositionOrdonnanceMap = new HashMap<String, Serializable>();
					transpositionOrdonnanceMap.put("ref", transpositionOrdonnanceRef);
					transpositionOrdonnance.setSerializableMap(transpositionOrdonnanceMap);
					listTranspositionOrdonnance.add(transpositionOrdonnance);
					dossier.setTranspositionOrdonnance(listTranspositionOrdonnance);
				}
			...
			if (StringUtils.isNotBlank(dirDeRef)) {
				...
				for (String transpositionDirectiveRef : transpositionDirectiveListRefList) {
					List<ComplexeType> listTranspositionDirectiveList = new ArrayList<ComplexeType>();
					ComplexeType transpositionDirective = new ComplexeTypeImpl();
					Map<String, Serializable> transpositionDirectiveMap = new HashMap<String, Serializable>();
					transpositionDirectiveMap.put("ref", transpositionDirectiveRef);
					transpositionDirective.setSerializableMap(transpositionDirectiveMap);
					listTranspositionDirectiveList.add(transpositionDirective);
					dossier.setTranspositionDirective(listTranspositionDirectiveList);
				}
			...
			eventProducer.fireEvent(inlineEventContext
					.newEvent(SolonEpgANEventConstants.ACTIVITE_NORMATIVE_ATTACH_EVENT));
	...
	public Dossier createDossier(CoreSession session, Mailbox currentMailbox, Dossier dossier, String posteId)
		...
		try {
			EntiteNode ministere = SolonEpgServiceLocator.getEpgOrganigrammeService().getMinistereFromNor(
					dossier.getNumeroNor().substring(0, 3));
			if (ministere != null) {
		...
		try {
			...
			throw new LocalizedClientException("Creation des dossiers");
		}
