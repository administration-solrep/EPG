...
@Operation(id = AddMesureToLoi.ID, category = Constants.CAT_DOCUMENT, label = "Add mesure to loi", description = "Add mesure to loi")
	...
	public DocumentModel run() throws Exception {
		...
		try {
			...
			if (ministerePilote != null && ministerePilote.length() >= 3) {
				...
				if (ministere == null) {
					repriseLog.warn("ministere rapporteur pour mesure applicative not found (numeroOrdre "
							+ numeroOrdre + " - article = " + mesureApplicativeDTO.getArticle() + " ) => ministere "
							+ ministerePilote);
				} else {
					mesureApplicativeDTO.setMinisterePilote(ministere.getId());
				}
			} else {
				repriseLog.warn("ministere rapporteur pour mesure applicative not found (numeroOrdre " + numeroOrdre
						+ " - article = " + mesureApplicativeDTO.getArticle() + ") est vide");
			}

			mesureApplicativeDTO.setObservation(observation);

			MesureApplicative mesureApplicative = SolonEpgServiceLocator.getActiviteNormativeService()
