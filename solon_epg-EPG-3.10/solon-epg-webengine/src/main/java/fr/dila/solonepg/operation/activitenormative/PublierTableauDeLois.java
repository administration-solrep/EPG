...
@Operation(id = PublierTableauDeLois.ID, category = Constants.CAT_DOCUMENT, label = "Publier Tableau De Lois", description = "Publier Tableau De Lois")
public class PublierTableauDeLois {
    ...
    public void run() throws Exception {
        ...
        List<DocumentModel> documentModelList = SolonEpgServiceLocator.getActiviteNormativeService().getAllAplicationLoiDossiers(session,true);
        ActiviteNormative activiteNormative;
        for (DocumentModel documentModel : documentModelList) {
            activiteNormative = documentModel.getAdapter(ActiviteNormative.class);
            SolonEpgServiceLocator.getActiviteNormativeService().generateANRepartitionMinistereHtml(session, activiteNormative,true);
        }
