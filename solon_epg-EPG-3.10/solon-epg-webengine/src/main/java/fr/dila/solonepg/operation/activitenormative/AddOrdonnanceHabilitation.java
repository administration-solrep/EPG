...
@Operation(id = AddOrdonnanceHabilitation.ID, category = Constants.CAT_DOCUMENT, label = "Add Ordonnance Habilitation", description = "Add Ordonnance Habilitation")
	...
	public DocumentModel run() throws Exception {
		...
		try {
			...
			if (ministerePilote != null && ministerePilote.length() >= 3) {
				...
				if (ministere == null) {
					repriseLog.warn("ministere rapporteur pour ordonnance not found (numero ordonnance " + numero
			...
			Ordonnance ordonnace = SolonEpgServiceLocator.getActiviteNormativeService()
					.saveOrdonnanceHabilitationReprise(ordonnanceHabilitationDTO, article, loiHabilitationId, session,
							ratifie);

			session.save();
			if (ordonnace != null) {
				result = ordonnace.getDocument();
				repriseLog.debug("Ajout OrdonnanceHabilitation is OK  -> article = " + article + " - numeroNor "
						+ numeroNor);
			} else {
				repriseLog.debug("Ajout OrdonnanceHabilitation is KO  -> article = " + article + " - numeroNor "
						+ numeroNor);
			}
