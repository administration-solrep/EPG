 ...
 *
 */
@Name("fichePresentationDPGActions")
@Scope(ScopeType.CONVERSATION)
public class FichePresentationDPGActionsBean extends AbstractFichePresentationBean {
     ...
     */
    private static final STLogger LOGGER = STLogFactory.getLog(FichePresentationDPGActionsBean.class);
    ...
    protected void assignFiche() {
        try {
            ...
            FichePresentationDPG fichePresentation = SolonMgppServiceLocator.getDossierService().findOrCreateFicheDPG(this.documentManager, idDossier);
    ...
    public String saveFiche() {
        ...
        if (this.ficheDoc != null) {
            try {
                FichePresentationDPG fichePresentation = SolonMgppServiceLocator.getDossierService().saveFicheDPG(this.documentManager, this.ficheDoc);
    ...
    public String getIdDossier() {
        if (this.ficheDoc != null) {
            FichePresentationDPG fichePresentation = this.ficheDoc.getAdapter(FichePresentationDPG.class);
    ...
    public FicheReportsEnum getFicheReportsEnum() {
        return FicheReportsEnum.FICHE_DECLARATION_GENERALE;
    }
    
    public String genererCourier() throws Exception {
        return generationCourrierActions.genererCourier(currentCourier, ficheDoc, SolonMgppCourrierConstant.TABLE_VOC_COURRIER_DPG);
    }
    
    public String navigateToDetailsDPG(DocumentModel doc) throws ClientException {
        navigateToFicheDoc(doc, null) ;
        return SolonMgppViewConstant.VIEW_DETAILS_DPG ;
    } 
    
    public String navigateToFicheDPG(DocumentModel fDPGDoc, String contentViewName) throws ClientException {
        ...
        if (contentViewName != null) {
            ...
            if (currentPage != null && currentPage.contains(fDPGDoc)) {
                pageProvider.setCurrentEntry(fDPGDoc);
            }
        }
        
        FichePresentationDPG fiche = ficheDoc.getAdapter(FichePresentationDPG.class);
