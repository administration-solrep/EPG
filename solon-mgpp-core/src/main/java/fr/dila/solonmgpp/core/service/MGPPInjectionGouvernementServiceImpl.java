...
public class MGPPInjectionGouvernementServiceImpl implements MGPPInjectionGouvernementService {
    ...
    private static final STLogger LOGGER = STLogFactory.getLog(MGPPInjectionGouvernementService.class);
    ...
    private void initAllEntities(List<InjectionGvtDTO> listInjection) {
        ...
        for (InjectionGvtDTO entity : listInjection) {
            ...
            if (
                !entity.isGvt() &&
                entity.isaCreerSolon() &&
                !entity.isaModifierSolon() &&
     ...
     * @param newGov
     */
    private void addNewGovernment(CoreSession session, InjectionGvtDTO newGov) {
        ...
        String idGov =
            (
                gouvernementNode.getIdentifiant() == null
                    ? gouvernementNode.toGouvernementXsd().getId()
    ...
    public List<InjectionEpgGvtDTO> createComparedDTO(
        ...
        List<InjectionEpgGvtDTO> resultList
    ) {
        Map<String, InjectionGvtDTO> mapMinObtenus = SolonEpgServiceLocator
            .getEpgInjectionGouvernementService()
        ...
        for (InjectionGvtDTO newInject : listInjection) {
            ...
            if (newInject.isGvt()) {
                ...
                if (
                    !newInject.isaCreerSolon() &&
                    !newInject.isaModifierSolon() &&
                    !newInject.isNouvelleEntiteEPP() &&
    ...
    private boolean hasNewMinistere(InjectionGvtDTO newMin, CoreSession documentManager) {
        TableReferenceService tableRefService = SolonMgppServiceLocator.getTableReferenceService();
        if (newMin == null) {
            return false;
            ...
            if (
                tableRefService.getMinistereNode(
                    documentManager,
                    newMin.getLibelleLong(),
                    newMin.getFormule(),
                    newMin.getLibelleLong(),
                    newMin.getLibelleCourt()
                ) ==
                null
            ) {
                return true;
    ...
    private boolean hasNewIdentite(InjectionGvtDTO newMin, CoreSession documentManager) {
        TableReferenceService tableRefService = SolonMgppServiceLocator.getTableReferenceService();
        if (newMin == null) {
            return false;
    ...
    private boolean hasNewMandat(InjectionGvtDTO newMin) {
        ...
        return (
            (newMin.isaCreerReponses() && newMin.getNorEPP() == null) ||
            newMin.isaCreerSolon() ||
            newMin.isaModifierSolon()
        );
    }
     ...
     * @return
     */
    private MandatNode createMandat(CoreSession session, InjectionGvtDTO newMin) {
        Calendar cal = Calendar.getInstance();
     ...
     * @param newGov
     */
    private void createMinistereMandatIdentite(
        CoreSession session,
        List<InjectionGvtDTO> newEntities,
        InjectionGvtDTO newGov
    ) {
     ...
     * @param newGov
     */
    private void createMandatSolonModifier(
        CoreSession session,
        List<InjectionGvtDTO> newEntities,
        InjectionGvtDTO newGov
    ) {
        ...
        for (InjectionGvtDTO newEntity : newEntities) {
            ...
            MinistereNode ministereNode = tableRefService.getMinistereNode(
                session,
                newEntity.getLibelleLong(),
                newEntity.getFormule(),
                newEntity.getLibelleLong(),
                newEntity.getLibelleCourt()
            );
     ...
     * @param newGov
     */
    private void createMandatSolonCreer(
        CoreSession session,
        List<InjectionGvtDTO> newEntities,
        InjectionGvtDTO newGov
    ) {
