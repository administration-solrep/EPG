...
public class EvenementServiceImpl implements EvenementService {
    private static final String EXCEPTION_MSG_ERROR_COM_EPP =
    ...
    private static final String SOLON_MGPP_DOCUMENTS_TRANSMIS_AUX_ASSEMBLEES_SEQUENCER =
        "MGPP_DOCUMENTS_TRANSMIS_SEQUENCER_";
    private static final String DECLARATION_POILITIQUE_GENERALE = "PG";
    private static final String DECLARATION_SUJET_DETERMINE = "SD";
    private static final String JOURS_SUPPLEMENTAIRES_DE_SEANCE = "JSS";
    ...
    @Override
    public EvenementDTO findEvenement(
        final String idEvenement,
        final String numVersion,
    ...
    public EvenementDTO findEvenement(
        ...
        final CoreSession session,
        boolean gererFichePresentation,
        final boolean returnErrorIfNull
    ) {
        ...
        if (
            (
                chercherEvenementResponse == null ||
        ...
        if (chercherEvenementResponse == null) {
            throw new NuxeoException(EXCEPTION_MSG_ERROR_COM_EPP);
        } else if (
            chercherEvenementResponse.getStatut() == null ||
            !TraitementStatut.OK.equals(chercherEvenementResponse.getStatut())
        ) {
            NuxeoException clientExc = new NuxeoException(
                EXCEPTION_MSG_ERROR_COM_EPP +
        ...
        if (chercherEvenementResponse.getEvenement().size() > 1) {
            throw new NuxeoException(
                "La récupération de la version " + cacheVersion + " de la communication a retourné plusieurs résultats."
            );
        }
    ...
    @Override
    public EvenementDTO createEmptyEvenementDTO(final String typeEvenement, CoreSession session) {
        final EvenementDTO evenementDTO = new EvenementDTOImpl();
        final Map<String, Serializable> map = new HashMap<>();
        map.put(EvenementDTOImpl.NAME, typeEvenement);
        evenementDTO.put(EvenementDTOImpl.TYPE_EVENEMENT, (Serializable) map);
    ...
    @Override
    public EppEvtContainer createEppEvtContainerFromEvenementDTO(
        final EvenementDTO evenementDTO,
        final Boolean hasContenu,
        CoreSession session
    ) {
        final EppBaseEvenement eppBaseEvenement = ContainerBuilder
            .getInstance()
        ...
        if (eppBaseEvenement != null) {
            PieceJointeBuilder.getInstance().buildPieceJointeFromEvenementDTO(evenementDTO, eppBaseEvenement, session);

            final EppEvtContainer eppEvtContainer = new EppEvtContainer();
            setEppEvtInEppEvtContainerAndAssignType(
                eppBaseEvenement,
                eppEvtContainer,
     ...
     * @param string
     */
    private void setEppEvtInEppEvtContainerAndAssignType(
        final EppBaseEvenement eppBaseEvenement,
    ...
    private Boolean setTypeInContainer(
        final EppBaseEvenement eppBaseEvenement,
        final EppEvtContainer eppEvtContainer,
        Boolean set,
                                        ...
                                        try {
                                            ...
                                            try {
                                                final String type = evenementTypeEnum
                                                    ...
                                                    .replaceAll(BuilderConstant.NOT_ALPHA, "")
                                                    .replaceAll(
                                                        ContainerBuilder.QUINQUIES_LOWER,
    ...
    public EvenementDTO initialiserEvenement(final String typeEvenement, final CoreSession session) {
        return initialiserEvenement(typeEvenement, true, session);
    }

    @Override
    public EvenementDTO initialiserEvenement(
        final String typeEvenement,
        boolean genererIdDossier,
        final CoreSession session
    ) {
        return initialiserEvenementSuccessif(null, typeEvenement, genererIdDossier, session);
    }
    ...
    public void createEvenement(final EvenementDTO evenementDTO, final boolean publie, final CoreSession session) {
        ...
        final EppEvtContainer eppEvtContainer = createEppEvtContainerFromEvenementDTO(
            evenementDTO,
            Boolean.TRUE,
            session
        );
        ...
        if (creerVersionResponse == null) {
            ...
            creerVersionResponse.getStatut() == null || !TraitementStatut.OK.equals(creerVersionResponse.getStatut())
        ) {
            NuxeoException clientExc = new NuxeoException(
                "Erreur de communication avec SOLON EPP, la création de la communication a échoué." +
        ...
        EvenementBuilder
            .getInstance()
            .buildEvenementDTOFromContainer(evenementDTO, creerVersionResponse.getEvenement(), session);
        SolonMgppServiceLocator
            .getDossierService()
            .gererFichePresentation(creerVersionResponse.getEvenement(), publie, session);

        // Création de la navette si besoin
        SolonMgppServiceLocator.getNavetteService().addNavetteToFicheLoi(session, creerVersionResponse.getEvenement());
    }

    private void creerDeuxiemeCommunication(
        final EvenementDTO evenementDTO,
        final CoreSession session,
        final WSEvenement wsEvenement,
        final CreerVersionRequest creerVersionRequest,
        final EppEvtContainer eppEvtContainer
    ) {
        // pour une alerte à poser creation de 2 communications
        boolean ajouterCopie = false;
        ...
        if (evtAlerte != null && evtAlerte.isPositionAlerte()) {
            ...
            if (creerVersionResponse == null) {
                ...
                creerVersionResponse.getStatut() == null ||
                !TraitementStatut.OK.equals(creerVersionResponse.getStatut())
            ) {
                NuxeoException cltExp = new NuxeoException(
                    "Erreur de communication avec SOLON EPP, la création de la communication a échoué." +
                    WSErrorHelper.buildCleanMessage(creerVersionResponse.getMessageErreur())
                );
    ...
    private CreationType extractModeCreation(
        final EvenementDTO evenementDTO,
        ...
        try {
            final ModeCreationVersion modeCreation =
                (
                    evenementDTO.getVersionCouranteModeCreation() == null
                        ? null
            ...
            LOGGER.warn(
                session,
                MgppLogEnumImpl.FAIL_GET_MODE_CREATION_TEC,
                evenementDTO.getVersionCouranteModeCreation(),
                e
            );
            creationType = publie ? CreationType.PUBLIER : CreationType.CREER_BROUILLON;
        }
    ...
    public void createEvenementEppEvt01Brouillon(final Dossier dossier, final CoreSession session) {
        ...
        if (chercherDossierResponse == null) {
            ...
            (
                TraitementStatut.KO.equals(chercherDossierResponse.getStatut()) &&
                chercherDossierResponse.getMessageErreur().contains(DOSSIER_NOT_FOUND_EXCEPTION)
            ) ||
            (
                TraitementStatut.OK.equals(chercherDossierResponse.getStatut()) &&
                chercherDossierResponse.getDossier().isEmpty()
            )
        ) {
            ...
            // initialisation
            final EvenementDTO evenementDTO = initialiserEvenement(
                TypeEvenementConstants.TYPE_EVENEMENT_EVT01,
                session
            );
    ...
    @Override
    public EvenementDTO initialiserEvenementSuccessif(
        EvenementDTO evenementDTOPrecedent,
        String currentTypeEvenement,
        CoreSession session
    ) {
        return initialiserEvenementSuccessif(evenementDTOPrecedent, currentTypeEvenement, true, session);
    }

    @Override
    public EvenementDTO initialiserEvenementSuccessif(
        final EvenementDTO evenementDTOPrecedent,
        final String typeEvenement,
        boolean genererIdDossier,
        final CoreSession session
    ) {
        if (StringUtils.isBlank(typeEvenement)) {
            throw new NuxeoException("Le type de communication est obligatoire");
        }
        ...
        if (evenementType == null) {
            throw new NuxeoException(
                "Le type de communication n'a pas été trouvé dans SOLON EPP (" + typeEvenement + ")"
            );
        }
        ...
        if (initialiserEvenementResponse == null) {
            ...
            initialiserEvenementResponse.getStatut() == null ||
        ...
        if (eppEvtContainer.getEvt01() != null) {
            ...
            // de dépôt
            final FicheLoi ficheLoi = SolonMgppServiceLocator
                .getDossierService()
                .findOrCreateFicheLoi(session, eppEvt10.getIdDossier());
            ...
            if (StringUtils.isNotEmpty(ficheLoi.getAssembleeDepot())) {
                try {
                    ...
                    final MetaDonneesDescriptor metaDonneesDescriptor = SolonMgppServiceLocator
                        .getMetaDonneesService()
                        .getEvenementType(TypeEvenementConstants.TYPE_EVENEMENT_EVT10);

                    if (metaDonneesDescriptor != null) {
                        final Map<String, PropertyDescriptor> mapProperty = metaDonneesDescriptor
                            .getEvenement()
                            .getProperty();

                        // si emetteur et destinataire sont settes on met les autres en copie
                        final PropertyDescriptor pCopie = mapProperty.get(EvenementDTOImpl.DESTINATAIRE_COPIE);
                        if (pCopie != null) {
            ...
            // RG-SD-CRE-02
            if (
                evenementDTOPrecedent != null &&
                TypeEvenementConstants.SD01.equals(evenementDTOPrecedent.getTypeEvenementName()) &&
                eppEvtContainer.getSD02() != null
            ) {
                EppBaseEvenement eppBaseEvenementPrecedent = buildEppEvtFromEvenementDTO(
                    evenementDTOPrecedent,
                    session
                );
            ...
            if (
                ...
                if (eppBaseEvenementPrecedent instanceof EppPG01) {
                    ...
                    FichePresentationDPG ficheDPG = SolonMgppServiceLocator
                        .getDossierService()
    ...
    public void rectifierEvenement(final EvenementDTO evenementDTO, final boolean publier, final CoreSession session) {
        ...
        final CreerVersionRequest creerVersionRequest = new CreerVersionRequest();
        creerVersionRequest.setModeCreation(
            publier ? CreationType.RECTIFIER_PUBLIER : CreationType.RECTIFIER_BROUILLON
        );
        ...
        SolonMgppServiceLocator
            .getDossierService()
            .gererFichePresentation(creerVersionResponse.getEvenement(), publier, session);
    }
    ...
    public void completerEvenement(final EvenementDTO evenementDTO, final boolean publier, final CoreSession session) {
        ...
        final CreerVersionRequest creerVersionRequest = new CreerVersionRequest();
        creerVersionRequest.setModeCreation(
            publier ? CreationType.COMPLETER_PUBLIER : CreationType.COMPLETER_BROUILLON
        );
    ...
    public void traiterEvenement(
        final String etatEvenement,
        final EvenementDTO evenementDTO,
        final CoreSession session,
        final DossierLink dossierLink
    ) {
        if (evenementDTO == null) {
            throw new NuxeoException("Impossible de traiter cette communication, communication vide");
        }
        ...
        if (notifierTransitionResponse == null) {
            ...
            notifierTransitionResponse.getStatut() == null ||
        ...
        // RG-EVT-TRA-06
        if (EtatEvenement.ANNULE.value().equals(etatEvenement)) {
            return;
        } else {
            final DossierService dossierService = SolonMgppServiceLocator.getDossierService();
        ...
        if (evenementDTO.getTypeEvenementName().equals(TypeEvenementConstants.TYPE_EVENEMENT_EVT51)) {
            final FichePresentationOEP ficheOEP = SolonMgppServiceLocator
                .getDossierService()
            ...
            if (ficheOEP != null) {
                ...
                if (eppBaseEvenement instanceof EppEvt51) {
                    ...
                    final Calendar dateJO = eppEvt51.getDateJo() != null
                        ? eppEvt51.getDateJo().toGregorianCalendar()
                        : null;

                    if (Institution.ASSEMBLEE_NATIONALE.equals(eppEvt51.getEmetteur())) {
                        final List<DocumentModel> listRepresentant = dossierService.fetchRepresentantOEP(
                            session,
                            VocabularyConstants.REPRESENTANT_TYPE_AN,
                            ficheOEP.getDocument().getId()
                        );
                        createRepresentantFromEvt51(
                            session,
                            ficheOEP,
                            listRepresentant,
                            eppEvt51.getTitulaires(),
                            VocabularyConstants.REPRESENTANT_TYPE_AN,
                            VocabularyConstants.FONCTION_TITULAIRE_ID,
                            dateJO
                        );
                        createRepresentantFromEvt51(
                            session,
                            ficheOEP,
                            listRepresentant,
                            eppEvt51.getSuppleant(),
                            VocabularyConstants.REPRESENTANT_TYPE_AN,
                            VocabularyConstants.FONCTION_SUPPLEANT_ID,
                            dateJO
                        );
                        session.save();
        ...
        if (TypeEvenementConstants.isEvenementPG(evenementDTO.getTypeEvenementName())) {
            ...
            if (eppBaseEvenement instanceof EppPG04) {
                ...
                FichePresentationDPG fichePresentationDPG = dossierService.findFichePresentationDPG(
                    session,
        ...
        if (TypeEvenementConstants.SD01.equals(evenementDTO.getTypeEvenementName())) {
            ...
            if (eppBaseEvenement instanceof EppSD01) {
                ...
                FichePresentationSD fichePresentationSD = dossierService.findOrCreateFicheSD(
                    session,
        ...
        if (TypeEvenementConstants.JSS02.equals(evenementDTO.getTypeEvenementName())) {
            ...
            if (eppBaseEvenement instanceof EppJSS02) {
                ...
                FichePresentationJSS fichePresentationJSS = dossierService.findOrCreateFicheJSS(
                    session,
                    evenementDTO.getIdDossier()
                );
    ...
    private void createRepresentantFromEvt51(
        final CoreSession session,
        final FichePresentationOEP fiche,
        final List<DocumentModel> listRepresentant,
        final List<Mandat> mandats,
        ...
        for (final Mandat mandat : mandats) {
            ...
            if (!found) {
                ...
                modelDesired.setPathInfo(
                    fiche.getDocument().getPathAsString(),
    ...
    @Override
    public void accepterVersion(final String idEvenement, final CoreSession session) {
        WSEvenement wsEvenement = null;
        ...
        if (validerVersionResponse == null) {
            ...
            validerVersionResponse.getStatut() == null ||
    ...
    @Override
    public void rejeterVersion(final String idEvenement, final CoreSession session) {
        WSEvenement wsEvenement = null;
        ...
        if (validerVersionResponse == null) {
            throw new NuxeoException("Erreur de communication avec SOLON EPP, le rejet de la communication a échoué.");
        } else if (
            validerVersionResponse.getStatut() == null ||
            !TraitementStatut.OK.equals(validerVersionResponse.getStatut())
        ) {
    ...
    private void accuserReception(
        final String idEvenement,
        Integer majeur,
        final Integer mineur,
        final CoreSession session,
        final EvenementType evenementType
    ) {
        WSEvenement wsEvenement = null;
        ...
        if (accuserReceptionResponse == null) {
            throw new NuxeoException("Erreur de communication avec SOLON EPP, l'AR de la communication a échoué.");
        } else if (
            accuserReceptionResponse.getStatut() == null ||
        ...
        if (evenementType != null && evenementType.equals(EvenementType.EVT_24)) {
            ...
            for (final EppEvtContainer eppEvtContainer : chercherEvenementResponse.getEvenement()) {
                ...
                if (eppEvt24 != null) {
                    final FicheLoi ficheLoi = SolonMgppServiceLocator
                        .getDossierService()
                        .findOrCreateFicheLoi(session, eppEvt24.getIdDossier());
                    ...
                    if (
                        ...
                        versionCourante.getMajeur() == 1 &&
                        versionCourante.getMineur() == 0 &&
                    ...
                    try {
                        ...
                        dossier =
                            SolonEpgServiceLocator
                                .getDossierService()
                                .findDossierFromIdDossier(session, eppEvt24.getIdDossier());
                        if (dossier == null) {
                            // try by nor
                            dossier =
                                SolonEpgServiceLocator
                                    .getNORService()
                                    .findDossierFromNOR(session, eppEvt24.getIdDossier());
                        }
                        ...
                        LOGGER.error(
                            ...
                            "mise a jour de l'adoption impossible",
                            e
                        );
                    }
        ...
        if (EvenementType.EVT_39.equals(evenementType) || EvenementType.EVT_43_BIS.equals(evenementType)) {
            LOGGER.info(
                session,
                MgppLogEnumImpl.CREATE_FICHE_LOI_TEC,
    ...
    @Override
    public void abandonnerVersion(final String idEvenement, final CoreSession session) {
        WSEvenement wsEvenement = null;
        ...
        if (validerVersionResponse == null) {
            throw new NuxeoException("Erreur de communication avec SOLON EPP, l'abandon de la communication a échoué.");
        } else if (
            validerVersionResponse.getStatut() == null ||
            !TraitementStatut.OK.equals(validerVersionResponse.getStatut())
        ) {
    ...
    @Override
    public void annulerEvenement(final String idEvenement, final CoreSession session) {
        WSEvenement wsEvenement = null;
        ...
        if (annulerEvenementResponse == null) {
            ...
            annulerEvenementResponse.getStatut() == null ||
    ...
    @Override
    public void supprimerEvenement(final EvenementDTO evenementDTO, final CoreSession session) {
        WSEvenement wsEvenement = null;
        ...
        if (supprimerVersionResponse == null) {
            ...
            supprimerVersionResponse.getStatut() == null ||
    ...
    public void createEvenementEppEvt28Brouillon(final Dossier dossier, final CoreSession session) {
        ...
        if (rechercherEvenementResponse28 == null) {
            ...
            rechercherEvenementResponse28.getStatut() == null ||
        ...
        if (rechercherEvenementResponse28.getMessage().isEmpty()) {
            ...
            if (rechercherEvenementResponse24 == null) {
                ...
                rechercherEvenementResponse24.getStatut() == null ||
            ...
            for (final Message message : rechercherEvenementResponse24.getMessage()) {
                if (
                    ...
                    DateUtil
                        .xmlGregorianCalendarToDate(last24.getDateEvenement())
            ...
            if (last24 != null) {
                ...
                if (creerVersionResponseSenat == null) {
                    ...
                    creerVersionResponseSenat.getStatut() == null ||
    ...
    @Override
    public void createEvenementEppEvt36Brouillon(
        final CoreSession session,
    ...
    @Override
    public void createEvenementEppEvt49Brouillon(
        final CoreSession session,
        final FichePresentationOEP fichePresentationOEP
    ) {
        WSEvenement wsEvenement = null;
        ...
        final DossierService dossierService = SolonMgppServiceLocator.getDossierService();
        final List<DocumentModel> listRepresentantAN = dossierService.fetchRepresentantOEP(
            session,
            VocabularyConstants.REPRESENTANT_TYPE_AN,
        ...
        final List<DocumentModel> listRepresentantSE = dossierService.fetchRepresentantOEP(
            session,
            VocabularyConstants.REPRESENTANT_TYPE_SE,
            fichePresentationOEP.getDocument().getId()
        );
    ...
    @Override
    public void createEvenementEppEvt32Brouillon(
        final CoreSession session,
        final FichePresentationAVI fichePresentationAVI
    ) {
        WSEvenement wsEvenement = null;
    ...
    @Override
    public void createEvenementEppAUD01Brouillon(CoreSession session, FichePresentationAUD fichePresentationAUD) {
        WSEvenement wsEvenement = null;
        ...
        List<DocumentModel> listRepresentant = SolonMgppServiceLocator
            .getDossierService()
            .fetchPersonneAUD(session, fichePresentationAUD.getDocument().getId());
        ...
        if (listRepresentant != null && !listRepresentant.isEmpty()) {
            for (DocumentModel representant : listRepresentant) {
                ...
                if (
                    representantAUD.getDateFin() == null || representantAUD.getDateFin().before(Calendar.getInstance())
                ) {
    ...
    @Override
    public void createEvenementEppDOC01Brouillon(CoreSession session, FichePresentationDOC fichePresentationDOC) {
        WSEvenement wsEvenement = null;
    ...
    @Override
    public void createEvenementEppEvt29Brouillon(
        final CoreSession session,
        final FichePresentationDecret fichePresentationDecret
    ) {
        WSEvenement wsEvenement = null;
        ...
        final List<Message> messagesAN = rechercherMessage(
            session,
            wsEvenement,
            fichePresentationDecret.getNor(),
            EvenementType.EVT_29,
        ...
        final List<Message> messagesSenat = rechercherMessage(
            session,
            wsEvenement,
            fichePresentationDecret.getNor(),
            EvenementType.EVT_29,
    ...
    @Override
    public void createEvenementEppEvt30Brouillon(
        final CoreSession session,
        final FichePresentationDecret fichePresentationDecret
    ) {
        if (StringUtils.isEmpty(fichePresentationDecret.getNorOuvertureSessionExt())) {
            LOGGER.warn(session, MgppLogEnumImpl.EMPTY_NOR);
            throw new NuxeoException("Le champ NOR du Décret d'ouverture de la session extraordinaire est vide");
        }
        ...
        final List<Message> messagesAN = rechercherMessage(
            session,
            wsEvenement,
            fichePresentationDecret.getNorOuvertureSessionExt(),
            EvenementType.EVT_30,
            Institution.ASSEMBLEE_NATIONALE
        );
        ...
        if (messagesAN == null || messagesAN.size() == 0) {
            ...
            for (final Message message : messageList) {
                if (
                    ...
                    message.getDateEvenement() != null &&
                    DateUtil
                        .xmlGregorianCalendarToDate(last29.getDateEvenement())
                        .before(DateUtil.xmlGregorianCalendarToDate(message.getDateEvenement()))
                ) {
                    last29 = message;
                }
        ...
        if (messagesSenat == null || messagesSenat.size() == 0) {
            ...
            for (final Message message : messageList) {
                if (
                    ...
                    message.getDateEvenement() != null &&
                    DateUtil
                        .xmlGregorianCalendarToDate(last29Senat.getDateEvenement())
                        .before(DateUtil.xmlGregorianCalendarToDate(message.getDateEvenement()))
                ) {
                    last29Senat = message;
                }
    ...
    public void checkProcedureAcceleree(final CoreSession session, final EvenementDTO evenementDTO) {
        if (TypeEvenementConstants.TYPE_EVENEMENT_EVT19.equals(evenementDTO.getTypeEvenementName())) {
            ...
            final List<DocumentModel> navettes = SolonMgppServiceLocator
                .getNavetteService()
                .fetchNavette(session, fiche.getDocument().getId());
            ...
            if (listNiveauAn != null && listNiveauSenat != null) {
                if (listNiveauAn.size() == listNiveauSenat.size() && listNiveauAn.size() == 1) {
                    ...
                    if (fiche.getProcedureAcceleree() == null) {
                        throw new NuxeoException("La procédure accélérée doit être engagée.");
                    }
                }
            }
    ...
    @Override
    public void createEvenementEppEvt31Brouillon(
        final CoreSession session,
        final FichePresentationDecret fichePresentationDecret
    ) {
        if (StringUtils.isEmpty(fichePresentationDecret.getNorLoi())) {
            LOGGER.warn(session, MgppLogEnumImpl.EMPTY_NOR);
            throw new NuxeoException("Le champ NOR de la loi est vide");
        }
        ...
        final List<Message> messagesAN = rechercherMessage(
            session,
            wsEvenement,
            fichePresentationDecret.getNorLoi(),
            EvenementType.EVT_31,
            Institution.ASSEMBLEE_NATIONALE
        );
        List<Message> messageList = null;
        ...
        if (messagesAN == null || messagesAN.size() == 0) {
            ...
            for (final Message message : messageList) {
                if (
                    ...
                    message.getDateEvenement() != null &&
                    DateUtil
                        .xmlGregorianCalendarToDate(lastMessage.getDateEvenement())
                        .before(DateUtil.xmlGregorianCalendarToDate(message.getDateEvenement()))
                ) {
                    lastMessage = message;
                }
    ...
    @Override
    public void createEvenementEppEvt35Brouillon(
        final CoreSession session,
        final FichePresentationDecret fichePresentationDecret
    ) {
        WSEvenement wsEvenement = null;
    ...
    private List<Message> rechercherMessage(
        final CoreSession session,
        final WSEvenement wsEvenement,
        ...
        if (rechercherEvenementResponse01 == null) {
            ...
            rechercherEvenementResponse01.getStatut() == null ||
    ...
    public void mettreEnAttenteRelancer(CoreSession session, String idEvenement, boolean mettreEnAttente) {
        ...
        if (majInterneResponse == null) {
            ...
            majInterneResponse.getStatut() == null || !TraitementStatut.OK.equals(majInterneResponse.getStatut())
        ) {
            NuxeoException clientExc = new NuxeoException(
                EXCEPTION_MSG_ERROR_COM_EPP + WSErrorHelper.buildCleanMessage(majInterneResponse.getMessageErreur())
            );
            LOGGER.error(session, STLogEnumImpl.FAIL_CREATE_COMM_TEC, clientExc);
            throw clientExc;
        }
    ...
    public String generateIdDossierDOC() {
        return generateIdDossier(
            DOCUMENTS_TRANSMIS_AUX_ASSEMBLEES,
    ...
    private String generateIdDossierJSS() {
        return generateIdDossier(JOURS_SUPPLEMENTAIRES_DE_SEANCE, SOLON_MGPP_ARTICLE_28_3C_SEQUENCER);
    }
    ...
    private String generateIdDossier(String prefix, String sequence) {
        ...
        final String compteur = String.format("%04d", sequencer.getNextLong(sequence + annee) - 1);

        return idossier.append(prefix).append(annee).append(compteur).toString();
    }

    @Override
    public void enCoursDeTraitementEvenement(String idEvenement, CoreSession session) {
        if (idEvenement == null) {
            throw new NuxeoException("Impossible de traiter cette communication, communication vide");
        }
