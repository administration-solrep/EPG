...
public class ParapheurModelServiceImpl extends DefaultComponent implements ParapheurModelService {
    ...
    @Override
    public DocumentModel getParapheurModelRoot(final CoreSession session) {
        // création requete
        ...
        if (list == null || list.isEmpty()) {
            throw new NuxeoException("Racine des modèles de fond de dossier non trouvée");
        } else if (list.size() > 1) {
            throw new NuxeoException("Plusieurs racines des modèles de fond de dossier trouvées");
        }
        return list.get(0);
    }
        ...
        try {
            ...
            if (typeActe.isEmpty()) {
                throw new NuxeoException(
                    " erreur lors de la creation de l'arborescence du modele de parapheur => type acte null "
                );
            }
            ...
            List<String> idsFormatAutoriseTypeActeEtExtrait =
                DossierSolonEpgConstants.FormatAutoriseListExtendFolderParapheur;
            ...
            if (acteService.hasInjectionType(typeActe)) {
                ...
                extraitNonVide = true;
                acteNonVide = false;
            } else if (TypeActeConstants.TYPE_ACTE_ACCORD_COLLECTIF_PUBLIC.equals(typeActe)) {
                idsFormatAutoriseTypeActeEtExtrait =
                    ImmutableList.of(
            ...
            if (acteService.isRapportAuParlement(typeActe)) {
                createParapheurModelFolderElement(
                    parapheurModel,
                    session,
                    SolonEpgParapheurConstants.PARAPHEUR_FOLDER_RAPPORT_NAME,
                    acteNonVide,
                    nbMaxDocActeEtExtrait,
                    null
                );
            } else {
                createParapheurModelFolderElement(
            ...
            throw new NuxeoException(" erreur lors de la creation de l'arborescence du modele de parapheur ", e);
        }
    }

    @Override
    public DocumentModel createParapheurModele(
        ...
        try {
            ...
            DocumentModel parapheurModel = session.createDocumentModel(
     ...
     */
    protected DocumentModel createParapheurModelFolderElement(
        final DocumentModel documentRacine,
        final CoreSession session,
        final String name,
    ...
    @Override
    public DocumentModel getParapheurModelFromTypeActe(final CoreSession session, final String typeActe) {
        final String query = "SELECT m.ecm:uuid as id FROM ModeleParapheur as m WHERE m.mp:typeActe = ? ";

        final List<DocumentModel> modeleParapheurList = QueryUtils.doUFNXQLQueryAndFetchForDocuments(
            session,
            "ModeleParapheur",
            query,
            new Object[] { typeActe },
    ...
    @Override
    public List<ParapheurModelNode> getParapheurModelRootNode(
        final CoreSession session,
        final DocumentModel parapheurModel
    ) {
        ...
        try {
            ...
            throw new NuxeoException("erreur lors de la recuperation des repertoire  du modele de parapheur", e);
        }
     ...
     */
    private ParapheurModelNode getParapheurModelNodeFromDocumentModel(
    ...
    public List<ParapheurModelNode> getChildrenParapheurModelNode(final String documentId, final CoreSession session) {
        ...
        try {
            ...
            throw new NuxeoException("erreur lors de la recuperation de repertoire fils du modele de parapheur", e);
        }
    ...
    public DocumentModel editFolder(
        ...
        final CoreSession session,
        final String name,
        final Boolean estNonVide,
        final Long NbMaxDoc,
        final List<File> feuilleStyleFiles,
        final List<String> formatsAutorisesIds
    ) {
    ...
    @Override
    public DocumentModel saveFolder(
        final DocumentModel documentCourant,
        final CoreSession session,
        final String typeActe
    ) {
    ...
    @Override
    public DocumentModel saveFolder(final DocumentModel documentCourant, final CoreSession session) {
        return saveFolder(documentCourant, session, null);
    }

    @Override
    public DocumentModel updateFormatFile(final DocumentModel documentCourant, final List<String> formatFichier) {
        documentCourant.setProperty(
    ...
    @Override
    public List<String> getFormatFile(final DocumentModel documentCourant) {
        List<String> formatsFile = PropertyUtil.getStringListProperty(
            documentCourant,
            SolonEpgParapheurConstants.PARAPHEUR_MODEL_FOLDER_DOCUMENT_SCHEMA,
            SolonEpgParapheurConstants.PARAPHEUR_MODEL_FOLDER_FORMAT_AUTORISE_PROPERTY
        );
