...
public class SolonEpgJetonServiceImpl extends JetonServiceImpl {
     ...
     */
    private static final String QUERY_JETON_BY_ID_DOSS =
        ...
        " IN (";
    private static final String QUERY_JETON_BY_ID_DOSS_ORDER_BY =
        " ORDER BY jt." +
        STSchemaConstant.JETON_DOCUMENT_SCHEMA_PREFIX +
        ":" +
    ...
    protected List<DocumentModel> getJetonsInBasket(
        ...
        if (STWebserviceConstant.CHERCHER_MODIFICATION_DOSSIER.equals(typeWebservice)) {
            List<DocumentModel> jetonsDocsList = getJetonsByJetonNumber(
                session,
                NUMERO_PANIER,
                typeWebservice,
                owner,
    ...
    protected boolean updateJetons(
        ...
        if (
            STWebserviceConstant.CHERCHER_MODIFICATION_DOSSIER.equals(typeWebservice) ||
            jetonsDocsPanier.size() <= jetonResultSize.intValue()
        ) {
     ...
     * @return
     */
    private List<DocumentModel> getJetonsDossierRelatedInBasket(
        final CoreSession session,
        final List<DocumentModel> jetonsDocsList
    ) {
        ...
        for (Entry<String, Set<String>> entry : typeModifIdsDocsMap.entrySet()) {
            StringBuilder query = new StringBuilder(QUERY_JETON_BY_ID_DOSS)
                .append(StringHelper.getQuestionMark(entry.getValue().size()))
                .append(") ")
            ...
            List<DocumentModel> othersJetonsDocs = QueryUtils.doUFNXQLQueryAndFetchForDocuments(
                session,
                STConstant.JETON_DOC_TYPE,
                query.toString(),
                params.toArray()
