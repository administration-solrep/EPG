...
public class ExportDossierSuppressionConsultationListener extends AbstractExportDossierListener {
    private static final String EXPORT_FILENAME = FileUtils.generateCompleteXlsFilename(
        "Dossiers_à_supprimer_consultation"
    );

    public ExportDossierSuppressionConsultationListener() {
        super(SolonEpgEventConstant.DOSSIERS_SUPPRESSION_CONSULTATION_EXPORT_EVENT, EXPORT_FILENAME);
    }
    ...
    protected long countItemsToExport(CoreSession session, Map<String, Serializable> eventProperties) {
        ...
        return dossierService.countAllDossiersSuppressionConsultation(session);
    }
    ...
    protected DataSource processExport(CoreSession session, Map<String, Serializable> eventProperties) {
        ...
        List<EpgDossierListingDTO> itemsToExport = dossierService.getAllDossiersSuppressionConsultation(session);
