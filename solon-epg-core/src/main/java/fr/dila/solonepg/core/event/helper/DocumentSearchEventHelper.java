...
public final class DocumentSearchEventHelper {
    ...
    public static void raiseEvent(final CoreSession session, final DocumentModel documentModel) {
        ...
        eventProperties.put(SolonEpgEventConstant.RESULTAT_CONSULTE_CURRENT_PATH, userWorkspacePath);
        ...
        eventProducer.fireEvent(
            inlineEventContext.newEvent(SolonEpgEventConstant.CURRENT_DOCUMENT_SEARCH_CHANGED_EVENT)
        );
    }
    ...
    protected static EspaceRechercheService.DocKind docToKind(final DocumentModel documentToAdd) {
        ...
        } else if (documentToAdd.hasSchema(SSFeuilleRouteConstant.FEUILLE_ROUTE_SCHEMA)) {
