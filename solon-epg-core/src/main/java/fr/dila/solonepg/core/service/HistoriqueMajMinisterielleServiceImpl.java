...
public class HistoriqueMajMinisterielleServiceImpl implements HistoriqueMajMinisterielleService {
    ...
    @Override
    public void registerMajDossier(final CoreSession session, final DocumentModel dossierDoc) {
        // on récupère l'ancien document
        LOGGER.info(
            session,
            EpgLogEnumImpl.CREATE_MAJ_MIN_TEC,
            "Creation des différences ministérielles pour le dossier"
        );
        ...
        if (idDoc == null) {
            oldApplicationLoi = new ArrayList<>();
            oldTranspositionDirective = new ArrayList<>();
            oldTranspositionOrdonnance = new ArrayList<>();
        } else {
            final DocumentModel oldDocument = session.getDocument(new IdRef(idDoc));
            oldDossier = oldDocument.getAdapter(Dossier.class);
            oldApplicationLoi = new ArrayList<>(oldDossier.getApplicationLoi().getTranspositions());
            oldTranspositionDirective = new ArrayList<>(oldDossier.getTranspositionDirective().getTranspositions());
            oldTranspositionOrdonnance = new ArrayList<>(oldDossier.getTranspositionOrdonnance().getTranspositions());
        }
        ...
        createDifference(
            ...
            oldApplicationLoi,
            new ArrayList<>(currentDossier.getApplicationLoi().getTranspositions())
        );
        createDifference(
            session,
            currentDossier,
            MAJ_TARGET.APPLICATION_ORDONNANCE,
            oldTranspositionOrdonnance,
            new ArrayList<>(currentDossier.getTranspositionOrdonnance().getTranspositions())
        );
        createDifference(
            session,
            currentDossier,
            MAJ_TARGET.TRANSPOSITION,
            oldTranspositionDirective,
            new ArrayList<>(currentDossier.getTranspositionDirective().getTranspositions())
        );
        ...
        LOGGER.info(
            ...
            "Fin de création des différences ministérielles pour le dossier"
        );
    }

    @Override
    public void registerMajMesure(
        final CoreSession session,
        final TexteMaitre texteMaitre,
        final MesureApplicative existingMesure,
        final MesureApplicativeDTO updatedMesure
    ) {
        // on récupère l'ancien document
        LOGGER.info(
            session,
            EpgLogEnumImpl.CREATE_MAJ_MIN_TEC,
            "Creation des différences ministérielles pour la mesure"
        );

        boolean fromAppLoi = ActiviteNormativeConstants.MESURE_FILTER.equals(
            existingMesure.getAdapter(ActiviteNormative.class).getApplicationLoi()
        );
        boolean fromAppOrdo = ActiviteNormativeConstants.MESURE_FILTER.equals(
            existingMesure.getAdapter(ActiviteNormative.class).getApplicationOrdonnance()
        );
        ...
        if (dossierDoc != null) {
            ...
            LOGGER.error(
                ...
                "Echec de récupération du dossier par le nor " + texteMaitre.getNumeroNor()
            );
        }
        LOGGER.info(
            session,
            STLogEnumImpl.LOG_INFO_TEC,
            "Fin de création des différences ministérielles pour la mesure"
        );
    }
     ...
     * @return la maj créée
     */
    private MajMin createMajMin(
        final CoreSession session,
        final Dossier dossier,
        final MAJ_TYPE modification,
        final MAJ_TARGET target,
        final ComplexeType genericTransp
    ) {
        final DocumentModel document = session.createDocumentModel(ActiviteNormativeConstants.MAJ_MIN_DOCUMENT_TYPE);
        document.setPathInfo(
            ActiviteNormativeConstants.MAJ_MIN_PATH + "/" + target.toString().toLowerCase(),
     ...
     * @return
     */
    private MajMin createMajMinMesure(
        final CoreSession session,
        final MesureApplicative existingMesure,
        final MesureApplicativeDTO updatedMesure,
        final Dossier dossier,
        ...
        if (!MODIF_MIN.equals(difference)) {
            final DocumentModel document = session.createDocumentModel(
    ...
    private String diffMesure(final MesureApplicative existingMesure, final MesureApplicativeDTO updatedMesure) {
        ...
        diff.append(
            diffString(
                existingMesure.getDirectionResponsable(),
                updatedMesure.getDirectionResponsable(),
        ...
        diff.append(
            diffString(
                existingMesure.getConsultationsHCE(),
                updatedMesure.getConsultationsHCE(),
                "Consultations obligatoires hors CE"
            )
        );
        if (size != diff.length()) {
            diff.append(TIRET);
        ...
        diff.append(
            diffString(
                existingMesure.getCalendrierConsultationsHCE(),
                updatedMesure.getCalendrierConsultationsHCE(),
                "Calendrier des consultations obligatoires hors CE"
            )
        );
        if (size != diff.length()) {
            diff.append(TIRET);
        ...
        diff.append(
            diffString(
                existingMesure.getCommunicationMinisterielle(),
                updatedMesure.getCommunicationMinisterielle(),
    ...
    private String diffString(String existingValue, String updatedValue, String label) {
        ...
        if (
            (StringUtils.isNotBlank(existingValue) && !existingValue.equals(updatedValue)) ||
            (StringUtils.isBlank(existingValue) && StringUtils.isNotBlank(updatedValue))
        ) {
    ...
    @Override
    public List<MajMin> createDifference(
        final CoreSession session,
        final Dossier dossier,
        final MAJ_TARGET target,
        final List<ComplexeType> initialList,
        final List<ComplexeType> finalList
    ) {
        final List<MajMin> results = new ArrayList<>();
        ...
        for (Entry<String, ComplexeType> entry : finalMap.entrySet()) {
            if (initialMap.containsKey(entry.getKey())) {
                ...
                if (!initialListComplexeType.equals(finalListComplexeType)) {
                    final MajMin modified = createMajMin(
                        session,
                        dossier,
                        MAJ_TYPE.MODIFICATION,
                        target,
                        finalListComplexeType
                    );
    ...
    @Override
    public List<DocumentModel> getHistoriqueMaj(final CoreSession session, final MAJ_TARGET target) {
        final DocumentModel doc = session.getDocument(getRef(target));
        final String selectMajMin =
            "SELECT * FROM MajMin WHERE ecm:parentId = '" +
            doc.getId() +
    ...
    @Override
    public List<Map<String, Serializable>> getHistoriqueMajMap(final CoreSession session, final MAJ_TARGET target) {
        ...
        for (final DocumentModel doc : docs) {
            ...
            String anId = SolonEpgServiceLocator
                ...
                .findActiviteNormativeIdByNumero(session, (String) mapForProvider.get("ref"));
    ...
    @Override
    public MajMin createMajMinHabilitation(
        final CoreSession session,
        final Dossier oldDossier,
        final Dossier newDossier
    ) {
        ...
        if (newRefLoi != null && !newRefLoi.equals("")) {
            ...
            document.setPathInfo(
                ActiviteNormativeConstants.MAJ_MIN_PATH + "/" + MAJ_TARGET.HABILITATION.toString().toLowerCase(),
                UUID.randomUUID().toString()
            );
            final DocumentModel documentCreated = session.createDocument(document);
            maj = documentCreated.getAdapter(MajMin.class);
