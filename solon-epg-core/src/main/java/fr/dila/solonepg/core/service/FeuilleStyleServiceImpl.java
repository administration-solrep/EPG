...
public class FeuilleStyleServiceImpl implements FeuilleStyleService {
    ...
    private static final Log log = LogFactory.getLog(FeuilleStyleServiceImpl.class);
     ...
     */
    private static final String GET_ALL_FEUILLES_STYLES_FOR_TYPE_ACTE_QUERY =
        "select r.ecm:uuid as id from ParapheurFolder AS r WHERE isChildOf(r.ecm:uuid, select f.ecm:uuid from ModeleParapheur AS f WHERE f.ecm:uuid = ? ) = 1";
    ...
    @Override
    public File checkFeuilleStyleValidAndTag(
        final File feuilleStyleFile,
        ...
        odfOfficeMeta.setUserDefinedData(
            OPENOFFICE_METADONNEE_NAME,
            MetaValueTypeAttribute.Value.STRING.toString(),
    ...
    @Override
    public String validateFeuilleStyle(
        final File feuilleStyleFile,
        final CoreSession session,
        final boolean isActeIntegral
    ) {
    ...
    @Override
    public Boolean checkFeuilleStyleValid(
        final File feuilleStyleFile,
        final String feuilleStyleName,
        final CoreSession session,
        final boolean isActeIntegral
    ) {
    ...
    protected OdfTextDocument checkOdfTextDocumentValid(
        final File feuilleStyleFile,
        final CoreSession session,
        final boolean isActeIntegral
    ) {
    ...
    @Override
    public String checkFichierParapheurValid(
        final File fichier,
        ...
        if (isParapheurTypeActeUpdated != null && isParapheurTypeActeUpdated) {
            log.info(
                "le type d'acte a été changé en cours de parcours : on neutralise les contrôles sur la feuille de style"
            );
            return null;
        }
        ...
        // autorisée
        if (
            idfeuillestyleList.size() < 1 &&
            (formatsAutorises == null || formatsAutorises.size() < 1) &&
            (dossierDoc.getIsDossierIssuInjection() == null || !dossierDoc.getIsDossierIssuInjection().booleanValue())
        ) {
        ...
        try {
            ...
            if (
                ...
                identifiantUniqueDocument.isEmpty() ||
    ...
    @Override
    public String updateDossierMetadataFromParapheurFile(
        final CoreSession session,
        final InputStream fichierStream,
        final String fileName,
        ...
        if (!parapheurService.isFolderExtraitOrFolderActe(repertoire, session)) {
            log.debug(
                "le fichier n'est pas ajouté dans l'un des répertoires acte ou extrait : on ne cherche pas à mettre à jour les données"
            );
            return null;
        }
        ...
        if (StringUtils.isNotEmpty(titreValue)) {
            ...
            try {
                ...
                if (StringUtils.isEmpty(dateSignatureValue)) {
                    ...
                    dateSignature.setTime(StringHelper.stringFormatWithLitteralMonthDateToDate(dateSignatureValue));
    ...
    public List<InfoFeuilleStyleFile> getFeuilleStyleListFromTypeActe(final String typeActe) throws Exception {
        ...
        try {
            ...
            try (CloseableCoreSession session = SessionUtil.openSession()) {
                ...
                final DocumentModel parapheurModel = parapheurModelService.getParapheurModelFromTypeActe(
                    session,
                    typeActe
                );
                ...
                final List<DocumentModel> feuilleStyleListDocModel = QueryUtils.doUnrestrictedUFNXQLQueryAndFetchForDocuments(
                    session,
                    GET_ALL_FEUILLES_STYLES_FOR_TYPE_ACTE_QUERY,
                    params
                );
            ...
            throw new NuxeoException("erreur lors de la récupération des feuilles de style");
        } finally {
                    ...
                    for (final Map<String, Serializable> fileMap : listFile) {
                        ...
                        if (pos != null && Integer.parseInt(pos) == iterateur) {
                            final String url = String.format(
                                URL_FILE,
                                docId,
                                "parapheur_folder_solon_epg:feuilleStyleFiles",
                                iterateur,
                                fileNameFile
                            );
