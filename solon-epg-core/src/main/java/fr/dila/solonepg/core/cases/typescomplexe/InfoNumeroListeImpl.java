...
public class InfoNumeroListeImpl extends ComplexeTypeImpl implements InfoNumeroListe {
    ...
    public InfoNumeroListeImpl(Map<String, Serializable> serializableMap) {
        if (
            serializableMap.get(
                TraitementPapierConstants.TRAITEMENT_PAPIER_DATE_GENERATION_LISTE_SIGNATURE_PROPERTY
            ) instanceof Calendar
        ) {
            this.dateGenerationListe =
        ...
        if (
            serializableMap.get(
                TraitementPapierConstants.TRAITEMENT_PAPIER_NUMERO_LISTE_SIGNATURE_PROPERTY
            ) instanceof String
        ) {
            this.numeroListe =
    ...
    public void setSerializableMap(Map<String, Serializable> serializableMap) {
        Serializable dateGenerationListe = serializableMap.get(
            TraitementPapierConstants.TRAITEMENT_PAPIER_DATE_GENERATION_LISTE_SIGNATURE_PROPERTY
        );
        Serializable numeroListe = serializableMap.get(
            TraitementPapierConstants.TRAITEMENT_PAPIER_NUMERO_LISTE_SIGNATURE_PROPERTY
        );
        //set the map
