...
public class FicheSignaletiqueOrdonnanceDTO implements Serializable {
    ...
    public FicheSignaletiqueOrdonnanceDTO(ActiviteNormative activiteNormative, CoreSession session) {
        ...
        if (texteMaitre.isDispositionHabilitation() == null) {
            ...
            ordonnanceDTO.setFondementConstitutionnel(
                texteMaitre.isDispositionHabilitation() ? ARTICLE_38_C : ARTICLE_74_1
            );
        }
        ...
        ordonnanceDTO.setDate(DateUtil.toDate(texteMaitre.getDateCreation()));
        ordonnanceDTO.setDatePublication(DateUtil.toDate(texteMaitre.getDatePublication()));
        ordonnanceDTO.setNumeroNor(texteMaitre.getNumeroNor());

        // loi de ratification
        ratificationDTO = new ArrayList<>();
        List<LoiDeRatification> listLoiRatification = activiteNormativeService.fetchLoiDeRatification(
            texteMaitre.getLoiRatificationIds(),
            session
        );
        ...
        for (LoiDeRatification loiDeRatification : listLoiRatification) {
            ...
            rDTO.setDateDepot(DateUtil.toDate(loiDeRatification.getDateDepot()));
            rDTO.setDateLimite(DateUtil.toDate(loiDeRatification.getDateLimiteDepot()));
            rDTO.setDatePromulgation(extractDatePromulgation(loiDeRatification.getTitreOfficiel()));
            rDTO.setDatePublication(DateUtil.toDate(loiDeRatification.getDatePublication()));
        ...
        // decret
        decretApplicationDTO = new ArrayList<>();
        ...
        for (Decret decret : listDecret) {
            ...
            dDTO.setDatePublication(DateUtil.toDate(decret.getDatePublication()));
