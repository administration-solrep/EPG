...
public class CloseTextesNonPubliesBatchListener extends AbstractBatchEventListener {
     ...
     * Logger
     */
    private static final STLogger LOGGER = STLogFactory.getLog(CloseTextesNonPubliesBatchListener.class);
    private static final String QUERY_TXT =
        "select d.ecm:uuid as id from Dossier as d where d.dos:typeActe = '" +
    ...
    protected void processEvent(CoreSession session, Event event) {
        ...
        int nbTextesClos = 0;
        try {
            String delaiCloture = paramServ.getParametreValue(
                session,
            ...
            Long count = QueryUtils.doCountQuery(
                session,
                QueryUtils.ufnxqlToFnxqlQuery(QUERY_TXT),
                new Object[] { today }
            );
            ...
            for (Long offset = (long) 0; offset <= count; offset += limit) {
                ...
                LOGGER.info(
                    session,
                    EpgLogEnumImpl.PROCESS_B_CLOSE_TXT_NON_PUB_TEC,
                    "Récupération des dossiers de " + offset + " à " + borne
                );
        ...
        try {
            STServiceLocator
                ...
                .createBatchResultFor(
                    batchLoggerModel,
                    "Clôture des textes non publiés au JO signés depuis " + delaiClotureInt + " jours",
                    nbTextesClos,
                    endTime - startTime
                );
