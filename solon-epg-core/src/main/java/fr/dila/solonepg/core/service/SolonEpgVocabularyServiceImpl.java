...
public class SolonEpgVocabularyServiceImpl extends VocabularyServiceImpl implements SolonEpgVocabularyService {
    ...
    public boolean checkData(String vocabularyDirectoryName, String columName, String valueToCheck) {
        ...
        log.debug(
            "VocabularyService checkData : " + vocabularyDirectoryName + " / " + columName + " / " + valueToCheck
        );
        ...
        try {
            ...
            if (directory != null) {
                ...
                DocumentModelList listEntries = vocabularySession.query(new QueryBuilder(), false);
                Object valueVocabulary = null;
                int nbLine = listEntries.size();
                for (int i = 0; i < nbLine; i++) {
                    DocumentModel documentModel = listEntries.get(i);
                    valueVocabulary = documentModel.getProperty(schemaName, columName);
    ...
    public List<DocumentModel> getDocumentModelListFromDirectory(String vocabularyDirectoryName) {
        ...
        try {
            vocabularySession = directoryService.open(vocabularyDirectoryName);
            vocabularyDocumentModelList = vocabularySession.query(new QueryBuilder(), false);
        } catch (Exception e) {
    ...
    @Override
    public List<VocabularyEntry> getVocabularyEntryListFromDirectory(
        ...
        for (DocumentModel docModel : docList) {
            vocEntryList.add(
                new VocabularyEntry(
                    PropertyUtil.getStringProperty(docModel, schemaVocabulaire, STVocabularyConstants.COLUMN_ID),
                    PropertyUtil.getStringProperty(docModel, schemaVocabulaire, STVocabularyConstants.COLUMN_LABEL)
                )
            );
        }
        return vocEntryList;
    ...
    @Override
    public List<DocumentModel> getDocumentModelListFromDirectoryExceptFromIdList(
        String vocabularyDirectoryName,
        ...
        try {
            ...
            for (String key : idValueList) {
                List<DocumentModel> tempVocabularyDocumentModelList = new ArrayList<>();
    ...
    @Override
    public List<DocumentModel> getDocumentModelListFromIdsList(
        String vocabularyDirectoryName,
        List<String> idValueList
    ) {
        final DirectoryService directoryService = STServiceLocator.getDirectoryService();
        log.debug(
            "VocabularyService getDocumentModelListFromIdsList : " + vocabularyDirectoryName + " / " + idValueList
        );
        List<DocumentModel> vocabularyDocumentModelList = new ArrayList<DocumentModel>();
        Session vocabularySession = null;
        if (idValueList == null || idValueList.size() < 1) {
    ...
    public String getIdFromLabel(String vocabularyDirectoryName, String labelValue, String labelColumName) {
        ...
        try {
            ...
            if (directory != null) {
                ...
                DocumentModelList vocabularyEntriesList = vocabularySession.query(new QueryBuilder(), false);
                String vocabularyLabeIterator;
                for (DocumentModel vocabularyIterator : vocabularyEntriesList) {
                    vocabularyLabeIterator =
                        PropertyUtil.getStringProperty(
                            vocabularyIterator,
                            schemaName,
     ...
     **/
    private List<String> getIndexList(String keyword, Session dirSession, String schemaName)
        throws DirectoryException, NuxeoException {
        ...
        for (DocumentModel doc : docListResult) {
            if (
                schemaName.equals(STVocabularyConstants.VOCABULARY) ||
                schemaName.equals(STVocabularyConstants.XVOCABULARY)
            ) {
                label = PropertyUtil.getStringProperty(doc, schemaName, VocabularyHelper.LABEL_FIELD);
    ...
    @Override
    public List<String> getLabelsFromDirectory(String vocabularyDirectoryName, String schemaVocabulaire) {
        if (schemaVocabulaire == null || schemaVocabulaire.isEmpty()) {
            schemaVocabulaire = STVocabularyConstants.VOCABULARY;
        }
        List<DocumentModel> docList = getDocumentModelListFromDirectory(vocabularyDirectoryName);
    ...
    @Override
    public List<String> getLabelsFromDirectory(String vocabularyDirectoryName) {
