...
public class ActiviteNormativeRechercheFixture {
    ...
    private void setUpLinksBetweenActiviteNormatives() {
    ...
    private void createLinkMesure(String intituleLoi, String intituleMesure) {
    ...
    private void createLinkMesures(String intituleLoi, List<TexteMaitre> mesures) {
        for (TexteMaitre mesure : mesures) {
            createLinkMesure(intituleLoi, getId(mesure));
        }
    }

    private void createLinkDecrets(String intituleMesure, List<TexteMaitre> decrets) {
        for (TexteMaitre decret : decrets) {
            createLinkDecret(intituleMesure, getId(decret));
        }
    }

    private void createLinkDecret(String intituleMesure, String intituleDecret) {
    ...
    public ActiviteNormative getActiviteNormative(String type, String intitule) {
        return getActiviteNormative(type, intitule, ActiviteNormative.class);
    }

    public TexteMaitre getTexteMaitre(String type, String intitule) {
        return getActiviteNormative(type, intitule, TexteMaitre.class);
    }

    public List<TexteMaitre> getTexteMaitres(String type, String intitule) {
        return getActiviteNormatives(type, intitule, TexteMaitre.class);
    }

    public MesureApplicative getMesureApplicative(String intitule) {
        return getActiviteNormative(MESURE, intitule, MesureApplicative.class);
    }

    public Decret getDecret(String intitule) {
        return getActiviteNormative(DECRET, intitule, Decret.class);
    }

    private <E> E getActiviteNormative(String type, String intitule, Class<E> adapter) {
        DocumentModelList docs = getActiviteNormativeDocs(type, intitule);
        return docs.get(0).getAdapter(adapter);
    }

    private DocumentModelList getActiviteNormativeDocs(String type, String intitule) {
        String query =
            "SELECT * FROM ActiviteNormative WHERE texm:observation = '" +
            type +
            "' AND ecm:name LIKE '" +
            intitule +
            "'";
    ...
    private <E> List<E> getActiviteNormatives(String type, String intitule, Class<E> adapter) {
        DocumentModelList docs = getActiviteNormativeDocs(type, intitule);
        return DocUtil.adapt(docs, adapter);
    }

    public ActiviteNormative getLoi() {
        return getActiviteNormative(LOI, "loi01");
    }

    public int getActiviteNormativeCount() {
