...
public abstract class FeuilleRouteServiceImpl implements SSFeuilleRouteService {
	...
	@Override
	public void addStepAfterReject(final CoreSession session, final String routingTaskId) throws ClientException {

		final DocumentModel currentStepDoc = session.getDocument(new IdRef(routingTaskId));
		final DocumentRouteStep currentStep = currentStepDoc.getAdapter(DocumentRouteStep.class);
		...
		while (loopCurrentIndex > 0) {
			...
			if (stepModel.hasFacet(DocumentRoutingConstants.ROUTE_STEP_FACET)) {
				final STRouteStep step = stepModel.getAdapter(STRouteStep.class);
		...
		if (loopCurrentIndex == 0) {
			...
			final DocumentRouteStep newCurrentStep = copyStep(session, currentStep);
			documentRoutingService.addRouteElementToRoute(currentStepDoc.getParentRef(), currentStepIndex + 1,
					newCurrentStep, session);

			// Copie du step précédemment trouvé
			final DocumentRouteStep step = stepModel.getAdapter(DocumentRouteStep.class);
			final DocumentRouteStep newStep = copyStep(session, step);
